...
# Audio is served from Fern's static asset root. MP3s under fern/docs/assets/
# resolve to /assets/... at the site root, which the component references via
# the fixture's `audio` field.
ASSET_DIR = REPO_ROOT / "fern" / "docs" / "assets" / "vad-events"
# Fixtures are TS modules (not JSON) — Fern's MDX bundler does not resolve
# relative JSON imports from custom React components.
