...
<Steps>
  <Step title="Get an access token (server-side)">
    Call [`POST /conversation/register-call`](/atoms/api-reference/api-reference/realtime-agent/register-call)
    from your backend with your API key. Set `agent_id`, and optionally `mode`
...
<CardGroup cols={2}>
  <Card title="Agent Web SDK reference" icon="js" href="/atoms/atoms-platform/integrate/web-socket-sdk">
    Full SDK API — config, methods, events, and a copy-paste smoke test.
  </Card>
  <Card title="Register Call" icon="key" href="/atoms/api-reference/api-reference/realtime-agent/register-call">
    Mint a short-lived access token for the WebSocket.
  </Card>
  <Card title="Realtime Agent WebSocket API" icon="plug" href="/atoms/api-reference/api-reference/realtime-agent/realtime-agent">
    The raw wire protocol behind the SDK — message types and payload shapes.
  </Card>
</CardGroup>
