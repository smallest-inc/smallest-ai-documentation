...
<Info>
  ...
  [Agent Web SDK](/voice-agents/platform/agent-sdk/web-socket-sdk) reference.
</Info>
...
| Event | When | Use |
|---|---|---|
| `session_started` | Call ready | Enable UI, store `call_id` (payload also includes `sample_rate` and `channels`) |
| `session_ended` | Call ended | Disable UI. `reason` is `client_requested`, `websocket_closed`, `setup_failed`, or a numeric close code. Fatal setup failures all close with the same code (`4000`) whether or not they're retryable, so check the `error` event fired just before the close for the real reason instead of branching on the close code |
...
<CardGroup cols={2}>
  <Card title="Agent Web SDK reference" icon="js" href="/voice-agents/platform/agent-sdk/web-socket-sdk">
