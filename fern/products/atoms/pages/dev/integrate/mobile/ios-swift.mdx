...
Native iOS apps connect to the Atoms agent using Apple's built-in WebSocket and audio APIs. No third-party packages required. The integration uses `URLSessionWebSocketTask` (available since iOS 13) for the wire protocol and `AVAudioEngine` for microphone capture and PCM playback.

<Note>
  The code on this page is compile-correct against the iOS 15+ SDK but has not been validated on a physical device. Verify microphone capture routing, interruption handling, and background behavior on your target devices before shipping.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Android (Kotlin)" icon="android" href="/atoms/developer-guide/integrate/mobile/android-kotlin">
    Native Android integration with OkHttp WebSocket and AudioRecord/AudioTrack.
  </Card>
