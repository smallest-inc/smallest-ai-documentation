...
<Info>
  Audio you send must be declared with the format token your recorder actually
  produces. See [Audio Formats](/voice-agents/platform/agent-sdk/audio-formats) for the tokens, the Opus framing rule,
  and why a mismatched rate degrades transcription without raising an error.
</Info>
