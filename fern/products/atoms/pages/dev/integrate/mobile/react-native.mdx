...
React Native integrates with the Atoms agent over the [raw WebSocket protocol](/voice-agents/api-reference/api-reference/realtime-agent/realtime-agent). The runtime's built-in `WebSocket` global handles transport, and a single audio library handles PCM16 capture and scheduled playback.

The browser [Agent Web SDK](/voice-agents/platform/developer/web-socket-sdk) cannot be used here. It calls `navigator.mediaDevices.getUserMedia` and the Web Audio API, both of which are DOM APIs and unavailable in the React Native JavaScript runtime.
...
For iOS-only apps with strict binary-size or battery budgets, prefer the [iOS (Swift)](/voice-agents/platform/developer/mobile-integrations/i-os-swift) native path. For Flutter, see the [Flutter](/voice-agents/platform/developer/mobile-integrations/flutter) guide.
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Web SDK" icon="code" href="/voice-agents/platform/developer/web-socket-sdk">
    The JavaScript SDK for browser runtimes.
  </Card>
  <Card title="iOS (Swift)" icon="apple" href="/voice-agents/platform/developer/mobile-integrations/i-os-swift">
