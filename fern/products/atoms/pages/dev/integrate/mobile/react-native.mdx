...
React Native integrates with the Atoms agent over the [raw WebSocket protocol](/voice-agents/api-reference/realtime-agent/realtime-agent). The runtime's built-in `WebSocket` global handles transport, and a single audio library handles PCM16 capture and scheduled playback.

The browser [Agent Web SDK](/voice-agents/platform/agent-sdk/web-socket-sdk) cannot be used here. It calls `navigator.mediaDevices.getUserMedia` and the Web Audio API, both of which are DOM APIs and unavailable in the React Native JavaScript runtime.
...
For iOS-only apps with strict binary-size or battery budgets, prefer the [iOS (Swift)](/voice-agents/platform/agent-sdk/mobile-integrations/i-os-swift) native path. For Flutter, see the [Flutter](/voice-agents/platform/agent-sdk/mobile-integrations/flutter) guide.
...
export default function App() {
  ...
  function handleServerEvent(raw: string) {
    ...
    switch (ev.type) {
      ...
      case 'transcript':            /* UI: render final transcript for ev.role */ break;
      case 'agent.start_talking':   /* UI: show "speaking" */ break;
      case 'agent.stop_talking':    /* UI: hide "speaking"  */ break;
...
The quickstart's switch handles the common ones, but here are all the confirmed event types with their meanings for reference:

| Event | What to do |
|---|---|
| `session.created` | Connection accepted; you can start streaming mic audio. Contains `session_id`, `call_id`, `sample_rate`, and `channels`. |
| `output_audio.delta` | Decode base64, schedule on the `AudioContext`. |
| `transcript` | Final transcript for a completed turn. `{ role: "user" \| "assistant", text: string }`. |
| `agent.start_talking` | The agent's TTS turn is starting. Show a speaking indicator; optionally mute the mic to cut self-feedback. |
| `agent.stop_talking` | The agent's TTS turn is done. Unmute if you muted. |
| `session.closed` | Session ended. `reason` is `client_requested`, `websocket_closed`, or `setup_failed`. |
| `interruption` | User barged in during the agent turn. Drop the playback queue and wait for a new `agent.start_talking`. Not part of the gateway's own typed contract, it's relayed verbatim from the STT/LLM/TTS worker, so treat the exact shape as provisional. |
| `error` | Server-side error during the session. `code` is open-ended (`invalid_json`, `invalid_event`, `no_session`, `internal_error`, `too_many_updates`, `worker_not_ready`, or an HTTP-status string). Non-fatal errors keep the socket open. |

<Note>
  Client → server, the gateway also accepts `input_text.send` (send a typed message any time, not just in a text-only mode) and `session.close` (ask for a clean shutdown). See [Sending text](#sending-text) and [Ending the session](#ending-the-session) below.
</Note>
...
3. Server sends `agent.start_talking`.
4. Server sends a stream of `output_audio.delta` chunks.
5. Server sends `agent.stop_talking`.
6. Back to step 1.

The client does not need to send `input_audio_buffer.commit` in normal conversational flow, it's currently a reserved no-op on the server (accepted but ignored) and never forces a turn boundary. The server's VAD handles turn boundaries entirely; there's no manual-commit capability to opt into today.

### Sending text

Send a typed message at any point in the session, whether or not the user is also speaking, with `input_text.send`:

```typescript
ws.send(JSON.stringify({ type: 'input_text.send', text: 'What are your hours?' }));
```

The only validation is a non-empty string; there's no documented length cap. The reply comes back the same way spoken input does, an `agent.start_talking` / `output_audio.delta` stream, followed by a `transcript` for the assistant turn.

### Ending the session

Closing the socket directly (`ws.close(1000, 'client end')`, as in the quickstart) works, but the server only sees it as `reason: "websocket_closed"`. For a session end that's distinguishable in your logs as client-initiated, send `session.close` first and let the server close in response:

```typescript
ws.send(JSON.stringify({ type: 'session.close' }));
// server replies with session.closed { reason: "client_requested" }, then closes the socket
```
...
When the server emits `interruption`, the user has spoken while the agent was talking. The agent's remaining TTS output for that turn is invalid; drop it. Call `playback.flush()` or your ring buffer's equivalent. A new `agent.start_talking` will follow shortly.
...
Don't branch reconnect logic on the WebSocket close code alone. Every fatal setup failure, whether transient (a dependency returning `503`) or permanent (agent not found, `404`), closes with the same code: `4000`. The close code can't tell you which one happened.

The signal that actually distinguishes them is the `error` event the server sends immediately before closing, whose `code` field is the underlying HTTP status as a string (or `internal_error`, `invalid_event`, etc. for protocol-level failures). Capture it and decide there:
...
ws.onmessage = (e) => {
  ...
  if (ev.type === 'error') lastErrorCode = ev.code;
  handleServerEvent(e.data as string);
};

ws.onclose = (e) => {
  if (e.code === 1000) { lastErrorCode = null; return; } // clean close, don't retry

  const retryable = lastErrorCode === null            // network drop, no error frame at all
    || lastErrorCode === 'internal_error'
    || lastErrorCode === 'worker_not_ready'
    || lastErrorCode === '503';
  lastErrorCode = null;
  if (!retryable) return; // e.g. "404" (agent not found), "400" (bad request): don't retry
...
Subscribe to the server's `error` event and surface non-transient errors to the user. Auth failures (invalid or expired token, unknown `agent_id`) are rejected as a plain HTTP `401`/`403` before the WebSocket upgrade completes, so they never show up here as an `error` message, they surface as `ws.onerror` firing with no prior `session.created`. Codes you can actually see in a mid-session `error` event include `invalid_event`, `no_session`, `internal_error`, `worker_not_ready`, and HTTP-status-as-string values like `404` (agent not found) or `503` (dependency unavailable):

```typescript
case "error":
  console.error(`[${ev.code}] ${ev.message}`);
  if (ev.code === "404") {
    // agent not found: not retryable, surface to the user
  }
  break;
```
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Web SDK" icon="code" href="/voice-agents/platform/agent-sdk/web-socket-sdk">
    The JavaScript SDK for browser runtimes.
  </Card>
  <Card title="iOS (Swift)" icon="apple" href="/voice-agents/platform/agent-sdk/mobile-integrations/i-os-swift">
    Native Swift integration with URLSessionWebSocketTask and AVAudioEngine.
  </Card>
  <Card title="Error reference" icon="triangle-exclamation" href="/voice-agents/platform/troubleshooting/error-reference">
