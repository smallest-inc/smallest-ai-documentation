...
The browser [Agent Web SDK](/voice-agents/platform/integrate/web-socket-sdk) cannot be used here. It calls `navigator.mediaDevices.getUserMedia` and the Web Audio API, both of which are DOM APIs and unavailable in the React Native JavaScript runtime.
...
For iOS-only apps with strict binary-size or battery budgets, prefer the [iOS (Swift)](/voice-agents/platform/developer/mobile-integrations/i-os-swift) native path. For Flutter, see the [Flutter](/voice-agents/platform/integrate/mobile-integrations/flutter) guide.
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Web SDK" icon="code" href="/voice-agents/platform/integrate/web-socket-sdk">
