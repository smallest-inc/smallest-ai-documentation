...
Flutter apps connect to the Atoms agent through the raw WebSocket protocol. Use the Dart `web_socket_channel` package for the wire, `mic_stream` for microphone PCM16 capture, and `flutter_pcm_sound` for low-latency PCM playback. The combination stays lightweight: no WebRTC, no LiveKit, no platform-specific WebRTC plugins.

<Note>
  The code on this page is compile-correct against the referenced packages but has not been validated on a physical device. Verify microphone permission flow, platform channel setup, and audio latency on your target devices before shipping.
</Note>
...
For single-platform native apps, the [iOS (Swift)](/atoms/developer-guide/integrate/mobile/ios-swift) and [Android (Kotlin)](/atoms/developer-guide/integrate/mobile/android-kotlin) guides give you full platform control with fewer intermediaries.
...
Future<void> _startMicStream() async {
  final micStream = await MicStream.microphone(
    audioSource:     AudioSource.VOICE_COMMUNICATION,  // Android AEC + NS
...
`AudioSource.VOICE_COMMUNICATION` selects the platform's echo-cancelled audio path on Android. On iOS, `mic_stream` uses the `AVAudioSessionModeVoiceChat` audio mode, which enables the equivalent pipeline.
...
void _handleServerEvent(dynamic raw) {
  ...
  switch (ev['type']) {
    ...
    case 'output_audio.delta':
      ...
      FlutterPcmSound.feed(PcmArrayInt16.fromUint8List(bytes));
      break;
    ...
    case 'agent_stop_talking':
      ...
      break;
    case 'interruption':
      FlutterPcmSound.clear();  // drop buffered agent audio
      break;
    case 'session.closed':
      _stop();
      break;
...
`mic_stream` configures `AVAudioSession` with the `.playAndRecord` category and `.voiceChat` mode. If your app uses other audio plugins (for example, `just_audio` for media playback), coordinate their session categories. Two plugins fighting over the session will cause one to silence the other.

### Android foreground service

If the call continues when the app is backgrounded, start a foreground service on the native Android side. `mic_stream` will continue capturing briefly when backgrounded but Android 12+ will revoke mic access within seconds without a foreground service declaring the `phoneCall` type. See the [Android guide](/atoms/developer-guide/integrate/mobile/android-kotlin) for the service implementation.
...
<CardGroup cols={2}>
  ...
  <Card title="Android (Kotlin)" icon="android" href="/atoms/developer-guide/integrate/mobile/android-kotlin">
    Native Android integration with OkHttp and AudioRecord/AudioTrack.
  </Card>
