...
Flutter applications integrate with the Atoms agent over the [raw WebSocket protocol](/voice-agents/api-reference/realtime-agent/realtime-agent).
...
For single-platform native apps, the [iOS (Swift)](/voice-agents/platform/agent-sdk/mobile-integrations/i-os-swift) guide gives you full platform control with fewer intermediaries.
...
void _handleServerEvent(dynamic raw) {
  ...
  switch (ev['type']) {
    case 'session.created':
      // update UI; ev['sample_rate'] and ev['channels'] confirm the negotiated audio format
      break;
    case 'output_audio.delta':
      ...
      break;
    case 'transcript':
      // ev['role'] is "user" or "assistant", ev['text'] is the final text
      break;
...
void _sendText(String text) {
  _channel?.sink.add(jsonEncode({'type': 'input_text.send', 'text': text}));
}

Future<void> _endSessionCleanly() async {
  _channel?.sink.add(jsonEncode({'type': 'session.close'}));
  // server replies with session.closed (reason: client_requested), then closes
}
```
...
The `onError`/`onDone` callbacks on the WebSocket stream fire when the connection drops or the server closes it. Don't decide retry vs. give-up from the close code alone: every fatal setup failure (agent not found, credits exhausted, a dependency timing out) closes with the same code, `4000`, whether or not it's actually retryable. Auth failures (bad token, unknown `agent_id`) never reach these callbacks at all, they're rejected as a plain HTTP `401`/`403` before the WebSocket upgrade completes, so `WebSocketChannel.connect` throws before you ever get a channel.

For a `4000` close, the only reliable retry signal is the `code` on the `error` message the server sends immediately before it:

```dart
String? _lastErrorCode;
int _retryMs = 500;

void _handleServerEventForRetry(Map<String, dynamic> ev) {
  if (ev['type'] == 'error') _lastErrorCode = ev['code'] as String?;
}

void _onWebSocketClosed(int? code) {
  final lastError = _lastErrorCode;
  _lastErrorCode = null;
  if (code == 1000) return; // clean close

  final retryable = lastError == null ||
      lastError == 'internal_error' ||
      lastError == 'worker_not_ready' ||
      lastError == '503';
  if (!retryable) return; // e.g. "404" (agent not found): don't retry
...
If your target devices have weaker echo cancellation, cancel the mic subscription on `agent.start_talking` and restart it on `agent.stop_talking`. The user's speech during the agent turn goes undetected; it is the safer trade-off than audible feedback.
