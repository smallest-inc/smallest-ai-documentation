...
<Tip>
  **In the browser, don't ship your raw API key.** Instead, mint a short-lived,
  single-use access token server-side with `POST /conversation/register-call`
  (see the **Register Call** endpoint under **Realtime Agent** in the API
  reference) and pass that token as `apiKey`. See the
  [Browser Voice Cookbook](/voice-agents/platform/agent-sdk/browser-voice-cookbook)
  for the full three-step flow. Passing a raw `sk_` key still works and is fine
  for server-side or trusted clients.
</Tip>
...
|---|---|---|
| `session_started` | `{ session_id: string, call_id: string, sample_rate: number, channels: number }` | The server accepts the connection and creates a session. Use `call_id` to look up the call log or subscribe to [live transcripts via SSE](/voice-agents/developer-guide/operate/analytics/sse-for-live-transcripts). `sample_rate` and `channels` reflect the negotiated audio format. |
| `session_ended` | `{ reason: string }` | The session has ended. `reason` is a string sent by the server: `client_requested`, `websocket_closed`, or `setup_failed`. When the WebSocket closes without a server-side `session.closed` frame reaching the client, you may instead see the numeric close code as a string (e.g. `"1006"`). Fatal setup failures (agent not found, credits exhausted, etc.) all close with the same code, `4000`, regardless of whether the failure is retryable, so don't branch reconnect logic on the close code itself. The `error` event fired immediately before the close carries the actual failure reason in its `code` field; branch on that instead. |
...
| `error` | `{ code: string, message: string }` | A non-fatal session error occurred. Fatal errors also trigger `session_ended`. `code` is open-ended, not a fixed set of values. Confirmed ones include `invalid_json`, `invalid_event`, `no_session`, `internal_error`, `too_many_updates`, `worker_not_ready`, and HTTP-status-as-string (e.g. `"404"`). |
