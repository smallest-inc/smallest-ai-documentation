...
interface AtomsAgentConfig {
  ...
  sampleRate?:     number;    // Deprecated. Sets the rate the SDK captures the microphone at; it is never sent to the server. The session's rates are resolved on /register-call. Prefer input_audio_format and output_audio_format there. See Audio Formats.
...
The `sampleRate` field predates the per-direction audio contract. It now only sets the rate the SDK opens its microphone at, and is never transmitted on the WebSocket connect on any auth path; the session's rates come from `/register-call`. Use [`output_audio_format`](/voice-agents/platform/agent-sdk/audio-formats) on `/register-call` instead; `input_audio_format` on the same call declares what your microphone actually produces.
