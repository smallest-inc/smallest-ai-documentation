...
interface AtomsAgentConfig {
  ...
  autoCaptureMic?: boolean;   // Default: true. Set false to start muted.
}
```
...
| `session_ended` | `{ reason: string }` | The session has ended. Common `reason` values: `client_requested`, `websocket_closed`, a server-side error tag. |
...
agent.on("error", (e) => {
  switch (e.code) {
    ...
    case "403":
      ...
      break;
    case "invalid_event":
      console.error("Malformed event sent by the SDK:", e.message);
      break;
    default:
...
try {
  ...
  // connect() rejects on WebSocket handshake failure, denied mic permission, etc.
