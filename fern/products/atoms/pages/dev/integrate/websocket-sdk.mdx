...
<Tip>
  **In the browser, don't ship your raw API key.** Instead, mint a short-lived,
  single-use access token server-side with
  [`POST /conversation/register-call`](/atoms/api-reference/api-reference/realtime-agent/register-call)
  and pass that token as `apiKey`. See the
  [Browser Voice Cookbook](/atoms/atoms-platform/integrate/browser-voice-cookbook)
