...
A JavaScript library that opens a live voice conversation with a Smallest agent from a web page. Published on npm as [`@smallest-ai/agent-sdk`](https://www.npmjs.com/package/@smallest-ai/agent-sdk). Wraps the [Realtime Agent WebSocket API](/voice-agents/api-reference/api-reference/realtime-agent/realtime-agent), plus microphone capture, PCM playback, and an event API.
...
| Runtime | Use |
|---|---|
| React Native, iOS, Android, Flutter | Talk to the [Realtime Agent WebSocket API](/voice-agents/api-reference/api-reference/realtime-agent/realtime-agent) directly. The SDK calls `navigator.mediaDevices.getUserMedia` and `AudioContext`, neither of which exists in those runtimes. |
...
<Tip>
  **In the browser, don't ship your raw API key.** Instead, mint a short-lived,
  ...
  [Browser Voice Cookbook](/voice-agents/platform/developer/browser-voice-cookbook)
...
For non-browser runtimes, connect to the [Realtime Agent WebSocket API](/voice-agents/api-reference/api-reference/realtime-agent/realtime-agent) directly. A raw-protocol guide with tested reference clients for Python, React Native, Swift, and Kotlin is in progress.

## Next steps

<CardGroup cols={3}>
  <Card title="Realtime Agent WebSocket API" icon="plug" href="/voice-agents/api-reference/api-reference/realtime-agent/realtime-agent">
