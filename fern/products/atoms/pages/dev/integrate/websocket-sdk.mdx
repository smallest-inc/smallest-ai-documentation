...
interface AtomsAgentConfig {
  ...
  sampleRate?:     number;    // Deprecated. Applies to raw-API-key auth only and is ignored when connecting with a short-lived access token — the rate is resolved on /register-call. Prefer output_audio_format on that call. See Audio Formats.
...
The `sampleRate` field predates the per-direction audio contract and only sets output. On the recommended access-token flow, the rate is pinned when `/register-call` runs and the SDK does not transmit `sampleRate` on the WebSocket connect. Use [`output_audio_format`](/voice-agents/platform/agent-sdk/audio-formats) on `/register-call` instead; `input_audio_format` on the same call declares what your microphone actually produces.
