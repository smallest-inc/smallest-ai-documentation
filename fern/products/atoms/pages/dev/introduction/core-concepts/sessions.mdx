...
The **Session** (`SwarmSession`) is the container that holds your active conversation. It creates a sandbox for every user connection, managing state, resources, and the event loop.

<Info>
  **One Session = One User**
  
  Every time a user connects via WebSocket (or phone), a new `SwarmSession` instance is spawned. This ensures total isolation—variables set in one session never leak to another.
</Info>
