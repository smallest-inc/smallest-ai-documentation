...
description: "Create, inspect, and place calls from your Atoms agents without leaving the terminal. Six commands, all verified against production."
---

The `smallestai agents` command group manages the agent itself: create new agents, list what you have, look up config, print a dashboard URL, check phone-number status, and place outbound calls. It sits alongside `smallestai agent-crew` (which manages crew code) and `smallestai auth` (which manages credentials). Same binary, three concerns.

<Note>
All six commands below were verified against `api.smallest.ai` on `smallestai==5.3.0`, including a real outbound call that reached the destination and completed successfully.
</Note>

## Setup

```bash
pip install smallestai>=5.3.0
```
...
<AccordionGroup>
  ...
  <Accordion title="agents phone-status: check phone-number attachment">
    ...
    Shows whether the agent has a phone number assigned. Useful sanity check before `agents call`: if `phone-status` says no number is attached, the outbound call will fail with a missing-caller-ID error. Rent or attach a number from **Deploy → Phone Numbers** first.
  </Accordion>
  ...
  <Accordion title="agents call: place an outbound call">
    ...
    **Example**
    ...
    Prints the call ID on success. Verified on production: the call reached the destination and `calls.get` returned `status=completed` with a real duration.

    <Note>
    **SDK gotcha (5.3.0):** `client.atoms.calls.get(...)` takes `id=<call_id>` as the keyword, not `conversation_id=`. A caller passing `conversation_id=` will get a `TypeError`. This is a SDK-side bug tracked for the next patch release.
    </Note>
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Versioning lifecycle (SDK)" href="/atoms/developer-guide/build/agents/versioning-lifecycle">
