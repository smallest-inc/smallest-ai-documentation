...
<AccordionGroup>
  ...
  <Accordion title="agents get: inspect one agent's config">
    ...
    Prints the full agent config. Includes prompt, voice, language, model, tools, timeouts, and every other field. What you see is the **active revision** on the live branch, which is the exact config used on the next call. See [Agent Versioning](/voice-agents/build/versioning) for the branch and revision model.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Build your agent" href="/voice-agents/build/build-your-agent">
    The ordered guide to configuring an agent from the dashboard.
  </Card>
  <Card title="Agent Versioning" href="/voice-agents/build/versioning">
