...
## Watch an in-progress call

<Info>
Requires `smallestai>=5.11.0` and an active session (`smallestai auth login`).
</Info>

Two `calls` subcommands stream a running call's activity into your terminal. Use them while a call is in progress; for a finished call, `smallestai calls transcript <call-id>` (no `--follow`) or `smallestai calls get <call-id>` returns the settled record.

```bash
smallestai calls events <call-id>               # full live event stream
smallestai calls transcript <call-id> --follow  # just the conversation, live
```

`calls events` streams transcript turns, per-turn latency, node transitions, tool calls, and errors. `calls transcript --follow` filters that stream down to conversation turns only. Add `--json` on `calls events` to emit one raw event per line for downstream tooling.

## Stream a build's logs

<Info>
Requires `smallestai>=5.11.0`.
</Info>

`agent-crew logs` tails a build's compile + deploy logs. Defaults to the latest build if you omit `[build-id]`.

```bash
smallestai agent-crew logs                # latest build for the linked agent
smallestai agent-crew logs <build-id>     # a specific build
```

Pairs with `smallestai agent-crew builds` (list + Make Live).

## Models command group (renamed from `waves`)

<Info>
Renamed in `smallestai>=5.11.0`.
</Info>

The speech command group is now `smallestai models` (text-to-speech, speech-to-text, voices). `waves` still works as a hidden, back-compatible alias, so existing scripts keep running; use `models` as the canonical name in new code and docs.

```bash
smallestai models tts synthesize --voice avery "Hello there."
smallestai models stt transcribe path/to/audio.wav
smallestai models voices list
```
