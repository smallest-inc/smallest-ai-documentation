...
subtitle: All 74 tools the Smallest MCP server provides.
description: 'Reference for all 74 tools the Smallest MCP server exposes. The AI editor picks the right tool from your natural-language prompt — you do not call these directly.'
---
...
| Tool | What it does |
|---|---|
| `add_agent_tool` | Add or update an API-call tool on a single-prompt agent — an HTTP request the agent invokes during a call (e.g. look up an order, book an appointment). Upserts by name and preserves existing tools. For versioned agents, creates a draft automatically. |
| `remove_agent_tool` | Remove a tool from an agent by name. For versioned agents, creates a draft automatically. |
| `set_pre_call_api` | Configure or disable an agent's pre-call API — one HTTP request made before the call connects to enrich the agent with variables (e.g. fetch a customer record by phone number). For versioned agents, creates a draft automatically. |
