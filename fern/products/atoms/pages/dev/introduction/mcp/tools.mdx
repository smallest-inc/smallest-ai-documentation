...
subtitle: All 79 tools the Smallest MCP server provides.
description: 'Reference for all 79 tools the Smallest MCP server exposes. The AI editor picks the right tool from your natural-language prompt — you do not call these directly.'
---
...
<Note>
Give at least one playbook an `end_call` tool (`add_end_call_tool: true`) — without one, the agent has no way to hang up when the caller says goodbye. `get_playbooks` warns about this.
</Note>

## Drafts

On versioned agents, every config edit lands in a draft rather than going live directly. Edit tools (`update_agent_prompt`, `add_agent_tool`, `set_pre_call_api`, `remove_agent_tool`, and the Playbooks tools `add_playbooks` / `update_playbook` / `configure_playbooks`) accept an optional `draft_id` so multiple changes — prompt, pre-call API, and a full toolset — can be stacked into **one** draft and shipped with a single `publish_draft`. When adding several API-call tools to the same draft, pass them as one batch: the draft's tools section is written wholesale, so the last write defines the full set.
