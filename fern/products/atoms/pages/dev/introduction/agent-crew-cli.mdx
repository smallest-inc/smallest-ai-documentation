...
<Steps>
  ...
  <Step title="Ship your env vars in a .env">
    <Warning>
      The deploy zip does **not** carry your shell environment — platform-side env-var injection isn't built yet. Drop your keys into a `.env` next to `server.py` so `load_dotenv()` finds them on the cloud:
      ...
      Without this, the deployed agent boots but stays silent on calls — the LLM client gets `api_key=None`, logs a warning to logs you can't see, and fails on the first request.
    </Warning>
  </Step>
  ...
  <Step title="Go Live">
    <Warning>
      A successful deploy is **not live by default**. Builds land in `SUCCEEDED` with `is_live=null` — your agent will not answer calls until you promote one.
    </Warning>
