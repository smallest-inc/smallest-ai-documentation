...
Before the CLI has anything to connect to, you need a Python file that defines a crew and starts a WebSocket server. Pick one of the two patterns below and `curl` the files into an empty directory of your choice — no `requirements.txt` to install, no `.env` file to set up. `smallestai` is the only dependency, and the examples read `OPENAI_API_KEY` straight from your shell.
...
```bash
curl -O https://raw.githubusercontent.com/smallest-inc/cookbook/main/voice-agents/background_agent/app.py
curl -O https://raw.githubusercontent.com/smallest-inc/cookbook/main/voice-agents/background_agent/support_agent.py
curl -O https://raw.githubusercontent.com/smallest-inc/cookbook/main/voice-agents/background_agent/sentiment_analyzer.py
```
