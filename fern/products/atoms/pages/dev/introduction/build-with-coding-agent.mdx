...
subtitle: Everything your coding agent (Claude Code, Codex, Cursor) needs to build on Smallest - machine-readable docs, the MCP server, and paste-ready build prompts.
description: "Copy-paste prompts and MCP setup that let a coding agent build a Smallest voice agent end to end, or wire Smallest models (Lightning TTS, Pulse STT, Electron LLM) into Pipecat, LiveKit, and other stacks."
---

Most people don't read docs page by page anymore. They paste a prompt into their AI editor and let it build. These two flows are made for that: pick the one that matches what you want, paste it into your coding agent's chat, and answer the questions it asks.

## Entry points for AI agents

- **Machine-readable docs:** [`/llms.txt`](https://docs.smallest.ai/llms.txt) (index) and [`/llms-full.txt`](https://docs.smallest.ai/llms-full.txt). Append `.md` to any docs URL to get clean markdown.
...
## Build with a prompt (copy-paste)
...
/ inbound / both. If they bring their own LLM or need custom per-turn logic, it's a
