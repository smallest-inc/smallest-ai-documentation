...
```bash
pip install "smallestai>=4.4.6,<4.4.7"
```

<Warning>
  **Version pin required (temporary).** `smallestai==4.4.7` ships a startup-validation hook that fails before any client connects when your `setup_handler` calls `await session.start()` — which every documented example does. Tracked as a regression to be fixed; until it's reverted, pin to `<4.4.7`. The cookbook examples below also need this pin.
</Warning>
...
# Pin to a working SDK version (see Installation warning above)
sed -i.bak 's/smallestai.*/smallestai>=4.4.6,<4.4.7/' requirements.txt
...
# Multi-node example needs 4.4.7+ — DON'T pin below it
pip install -r requirements.txt
```

<Warning>
  **Multi-node uses `smallestai>=4.4.7`** because `sentiment_analyzer.py` imports `SDKAgentLogEvent`, which was added in 4.4.7. On boot you'll see a `Startup validation failed — pod will not accept sessions` ERROR in logs from the SDK's `_validate_startup` hook. **The error is misleading — the pod does accept sessions and the agent works.** Tracked for an SDK fix; safe to ignore in the meantime.
</Warning>
