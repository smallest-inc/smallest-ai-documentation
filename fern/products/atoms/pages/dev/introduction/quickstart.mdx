...
<Warning>
  **Heads-up on a noisy boot log.** From `smallestai>=4.4.7`, you'll see this scary line at startup:

  ```
  ERROR | Startup validation failed — pod will not accept sessions. ValueError: Session not initialized
  ```

  Despite the message, **the pod does accept sessions and your agent works normally.** This is a SDK startup dry-run that runs your `setup_handler` against a null WebSocket to surface init-time errors; it always trips on `await session.start()` and logs the traceback. Real client connections handshake fine and your agent runs end-to-end. Verified on `smallestai==4.4.7` against the cookbook's `getting_started` example: `Application startup complete.` follows the ERROR, `Uvicorn` binds, `agent-crew chat` connects, session completes cleanly.

  If the ERROR mentions a missing API key (e.g. `OpenAIError: Missing credentials`) rather than `Session not initialized`, that **is** real — set `OPENAI_API_KEY` (or use Electron, see below) before booting.
</Warning>
...
Plus an LLM API key for the example below — pick one:

- **Electron** (Smallest's own LLM, OpenAI-compatible) — reuses `SMALLEST_API_KEY`. Set `base_url="https://api.smallest.ai/waves/v1"` and `model="electron"` in `OpenAIClient`. No extra key needed.
...
<Info>
  Multi-node uses `smallestai>=4.4.7` because `sentiment_analyzer.py` imports `SDKAgentLogEvent` (introduced in 4.4.7). Same boot-log caveat as the Installation section applies — you'll see the `Session not initialized` ERROR; the pod still serves.
</Info>
