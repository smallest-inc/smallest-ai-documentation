...
<Tip>
  Building in the dashboard instead, or want the design model for an agent (which settings to configure, in what order, and why)? See [Build your agent](/voice-agents/platform/create-agent/build-your-agent). This quickstart is the code path; that guide is the platform path.
</Tip>
...
<Note>
  **One harmless startup log on older `smallestai` versions.** On boot the server runs a quick self-check of your `setup_handler`. On versions up to the latest release, that self-check logs a single benign line and then serves normally:

  ```
  ERROR | Startup validation failed — pod will not accept sessions. ValueError: Session not initialized
  ```

  Ignore it: `Application startup complete.` follows, Uvicorn binds on `:8080`, and connections work. Upgrade to the latest `smallestai` to stop seeing it.

  A *different* error at that point is real. For example `OpenAIError: Missing credentials` means a node failed to construct. Set the missing credential (or point the node at your model) before booting.
</Note>
...
<Info>
  Multi-node uses `smallestai>=5.0.0` because `sentiment_analyzer.py` imports `SDKAgentLogEvent` (introduced in 4.4.7). Same harmless boot log as the Installation section applies on older versions (the `Session not initialized` line); the pod still serves, and upgrading removes it.
</Info>
...
<AccordionGroup>
  <Accordion title="What does my crew code own, and where do STT and TTS come from?">
    Your crew node owns the **LLM turn only**: it receives the transcript as messages and
    yields text. Speech-to-text (Pulse / Pulse Pro) and text-to-speech (Lightning) are set on
    the **agent in the dashboard**, not in code, and run platform-side. Turn-taking,
    interrupts, and telephony are platform-side too. So the pipeline is:

    ```
    caller audio -> Smallest STT -> your crew node (your LLM) -> text -> Smallest TTS -> caller
    ```

    You never write STT or TTS code. You pick them in agent settings; your node just generates
    the words.
  </Accordion>

  <Accordion title="Can I use my own LLM or runtime instead of an OpenAI model?">
    ...
    self.llm = OpenAIClient(
        ...
        base_url="https://your-runtime.example.com/v1",
    )
    ```
  ...
  <Accordion title="Can I build a multi-agent crew, or is it one agent per call?">
    Multi-agent is supported. A `CrewSession` runs multiple nodes in parallel for one call:

    - `OutputCrewNode`: user-facing, produces speech. You can have more than one (for example a
      router that hands off).
    - `BackgroundCrewNode`: observes every event silently (no audio), for example live
      sentiment, compliance checks, or logging. Output nodes can read a background node's state
      mid-call.

    The [`background_agent`](https://github.com/smallest-inc/cookbook/tree/main/voice-agents/background_agent)
    example shows a support agent plus a sentiment analyzer that triggers auto-escalation.
    Start single-node ([`getting_started`](https://github.com/smallest-inc/cookbook/tree/main/voice-agents/getting_started)),
    add nodes as you need them.
  </Accordion>

  <Accordion title="Can I test a full turn without a phone call?">
  ...
  <Accordion title="How do I set environment variables in production?">
    ...
    ```python
    # server.py (or wherever your entry point is)
  ...
  <Accordion title="What project layout does the cloud build expect?">
    ...
    - **Add a `requirements.txt`** if you want to keep a `pyproject.toml` for local tooling
      or editable installs. When both are present the builder uses `requirements.txt` and
      skips `pip install .` entirely.
  </Accordion>
</AccordionGroup>
