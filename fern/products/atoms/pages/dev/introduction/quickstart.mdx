...
<AccordionGroup>
  ...
  <Accordion title="What project layout does the cloud build expect?">
    ...
    A `src/` layout will **not** work. The cloud build runs `pip install .` on the uploaded
    bundle before copying source, and `src/`-layout projects install into `site-packages`
    where the entry-point loader can't find them.

    Same rule for `pyproject.toml`-only projects: if you keep a `pyproject.toml` (for local
    tooling, editable installs, etc.) also ship a `requirements.txt` alongside it. The
    cloud build reads `requirements.txt`.
  </Accordion>
</AccordionGroup>
