...
The connection happens at agent creation time. Pass the KB ID as `global_knowledge_base_id` to `create_agent`:

```python
agent = client.atoms.agents.create_agent(
    name="Support Agent",
    global_prompt="You are a helpful support agent. Use your knowledge base to answer product questions accurately.",
...
`get_agent()` returns the agent with its `globalKnowledgeBaseId` field:

```python
agent_details = client.atoms.agents.get_agent(id=agent_id)

linked_kb = agent_details.data.global_knowledge_base_id
...
Use `archive_agent()` to archive the agent, then `delete()` to remove the KB:
