...
The Atoms Agent Swarms SDK uses Loguru for logging. Set it up in your agent:
...
```python
class LoggingAgent(OutputSwarmNode):
...
class TimedAgent(OutputSwarmNode):
    async def generate_response(self):
        start = time.time()
        chunk_count = 0
...
class MetricsAgent(OutputSwarmNode):
    async def generate_response(self):
...
<CardGroup cols={2}>
  <Card title="Common Issues" icon="bug" href="/atoms/developer-guide/operate/testing-debugging/common-issues">
