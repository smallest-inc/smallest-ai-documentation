...
For the dashboard equivalent and a deeper conceptual overview, see [Agent Versioning](/voice-agents/build/versioning). For an endpoint-by-endpoint mapping from the v1 API, see the [v1 to v2 migration guide](/api-reference/deprecations/agent-versioning-migration).
...
Existing `versionId` values continue to resolve as `revisionId`, so IDs stored by earlier integrations keep working. Full endpoint-by-endpoint mapping and 409-handling patterns live in the [v1 → v2 migration guide](/api-reference/deprecations/agent-versioning-migration).
