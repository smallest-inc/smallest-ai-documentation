...
For the dashboard equivalent and a deeper conceptual overview, see [Agent Versioning](/voice-agents/platform/create-agent/agent-settings/versioning). For an endpoint-by-endpoint mapping from the v1 API, see the [v1 to v2 migration guide](/voice-agents/api-reference/versioning-v2-migration).
...
- **Every publish runs a security scan.** If the scan fails, the revision is not committed and the branch head does not move. The response returns `409` (or leaves the revision in `scanning` state depending on timing); fix the draft and publish again.
- Publishing on the live branch pushes to production immediately. Publish on a non-live branch first if you want to stage.
- **Restore does not overwrite the older revision.** It takes that older revision's config and publishes it as a **new** revision at the head of the branch. So a branch with `v1, v2, v3` restored to `v1` ends up with `v1, v2, v3, v4`, where `v4`'s content matches `v1` and `v4` is the head.
...
If you have code against `/agent/{id}/drafts/*` or `/agent/{id}/versions/*`, follow the [v1 → v2 migration guide](/voice-agents/api-reference/versioning-v2-migration). The mapping is straightforward: drafts move to `.../branches/{branchId}/draft`, publish moves to `.../branches/{branchId}/draft/publish`, and `activate` is replaced by `.../branches/{branchId}/live`. Existing `versionId` values continue to resolve as `revisionId`.
