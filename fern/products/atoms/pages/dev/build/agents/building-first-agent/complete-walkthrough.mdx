...
Open your terminal and install the Atoms Agent Crews SDK:
...
```bash
python -c "import smallestai; print('Atoms Agent Crews SDK installed')"
```
...
class OrderAssistant(OutputCrewNode):
...
MAX_TOOL_ROUNDS = 4  # safety cap on the tool-call loop
...
class OrderAssistant(OutputCrewNode):
    ...
    async def generate_response(self):
        """Generate a response. Loops if the LLM calls tools then needs another turn."""
...
1. Installed the Atoms Agent Crews SDK
...
<CardGroup cols={2}>
  <Card title="Prompts & Personality" icon="message" href="/voice-agents/developer-guide/build/agent-crews/llm/prompts">
    Craft better system prompts and agent personas.
  </Card>
  <Card title="Tools Deep Dive" icon="wrench" href="/voice-agents/developer-guide/build/agent-crews/tools/defining-tools">
