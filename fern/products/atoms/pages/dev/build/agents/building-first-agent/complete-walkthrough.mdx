...
Open your terminal and install the Atoms Agent Swarms SDK:
...
```bash
python -c "import smallestai; print('Atoms Agent Swarms SDK installed')"
```
...
class OrderAssistant(OutputSwarmNode):
...
from smallestai.atoms.swarm.nodes import OutputSwarmNode
from smallestai.atoms.swarm.clients.openai import OpenAIClient
from smallestai.atoms.swarm.tools.decorator import function_tool
from smallestai.atoms.swarm.tools.registry import ToolRegistry
...
if __name__ == "__main__":
    app = AtomsSwarmApp(setup_handler=setup)
    app.run()
```
...
1. Installed the Atoms Agent Swarms SDK
