...
Use `start_an_outbound_call()` to dial any phone number and connect it to your agent. This is the **production** outbound surface — full `variables` support for per-call personalization, optional caller-ID rotation, version pinning, and operator hooks.
...
These are the parameters the Python SDK's `start_an_outbound_call()` method accepts today.
...
The underlying `POST /conversation/outbound` endpoint accepts three additional parameters that are not yet exposed by the Python SDK. To use them today, call the REST API directly with `requests` or `httpx`:
...
| `operatorId` | string | Integration operator identifier (e.g. `"webengage"`) to trigger an operator-specific flow. |
| `operatorData` | object | Arbitrary payload forwarded to the operator. Used for `userId` / `journeyId` context in operator integrations. |
...
response = requests.post(
    ...
    json={
        ...
        "operatorId": "webengage",
        "operatorData": {"userId": "u_123", "journeyId": "j_abc"},
    },
)
...
Most production outbound flows need to drop the caller's name, the lead source, the product they were browsing, or a balance amount into the agent's opening line. Pass those values via `variables`:
...
In the agent's system prompt, reference them with double-brace syntax:

```text
Hi {{customer_name}}, this is Arya calling about your {{package}} subscription.
```

A few things to know:

- **Override semantics.** `variables` wins over the agent's `defaultVariables` for that one call. Variables you don't pass fall back to the default.
- **Scalar values only.** `string`, `number`, `boolean`. If you have nested data, flatten it before sending (`address_city` instead of `address: { city: ... }`).
- **Same field as the WebSocket realtime agent.** The Realtime Agent WebSocket accepts the same `variables` argument as a query parameter, so a flow that works on REST will work on WS with no schema changes.
- **Variables flow into webhooks.** The `post-conversation` webhook payload includes the resolved variables under `metadata.variables`, so your CRM ingestion can correlate the call against the row that triggered it.

See [Variables](/atoms/atoms-platform/features/variables) for the full guide to defining variables on the agent side.
