...
<Accordion title="Numbers read wrong or sentences look dropped in the transcript">
PII redaction on the agent config is rewriting numeric spans (`[ACCOUNTNUMBER_1]`, `[PHONE_1]`) before the LLM sees them. The audio itself is intact; only the transcript is masked. Turn redaction off on the agent config if the destination needs the raw numbers.
</Accordion>
