...
<Accordion title="Numbers read wrong, or sentences look dropped">
PII redaction on the agent config rewrites numeric spans to placeholders like `[ACCOUNTNUMBER_1]` or `[PHONE_1]` before the LLM sees them and before synthesis. Both the audio and the transcript use the masked form, so the caller hears the placeholder rather than the raw digits. Turn redaction off on the agent config if the destination needs the raw numbers spoken.
</Accordion>
