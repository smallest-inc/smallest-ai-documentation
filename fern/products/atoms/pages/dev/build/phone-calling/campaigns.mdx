...
3. **Phone Number ID** - From `client.atoms.phone_numbers.list()`

<Tip>
  Get your phone number ID using `client.atoms.phone_numbers.list()` - the `id` field on each item is what you pass to campaigns.
</Tip>
...
from smallestai.atoms.helpers import Audience, Campaign
...
# 2. Create agent
agent = client.atoms.agents.create_agent(
    name="Campaign Agent",
    global_prompt="You are a friendly assistant.",
...
| `phone_ids` | list | Yes | Phone number IDs (from `client.atoms.phone_numbers.list()`) |
| `description` | string | No | Campaign description |
| `max_retries` | int | No | Retry unanswered (0-10, default: 3) |
| `retry_delay` | int | No | Minutes between retries (1-1440, default: 15) |

<Warning>
Send `max_retries` and `retry_delay` as **integers, not strings**. Over the raw
REST API, `{"maxRetries": "3"}` returns `400 "Expected number, received
string"` — use `{"maxRetries": 3}`. Field names are case-sensitive and differ
by surface: SDK snake_case (`max_retries`), REST camelCase (`maxRetries`). A
wrong-case or misspelled key is silently ignored and defaults to 3.
</Warning>
...
phones = client.atoms.phone_numbers.list()
phone_id = phones.data[0].id
print(f"Using phone: {phones.data[0].attributes.phone_number}")

# 2. Create agent
agent = client.atoms.agents.create_agent(
    name=f"Campaign Agent {int(time.time())}",
    global_prompt="You are a friendly survey agent. Ask about interest and get their name.",
...
| Error | Solution |
|-------|----------|
| "No phone numbers available" | Pass `phone_ids` from `client.atoms.phone_numbers.list()` |
