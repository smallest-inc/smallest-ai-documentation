...
- `agent.start_talking`: fires once per agent turn, before the first `output_audio.delta` of that turn.
- `agent.stop_talking`: fires when the agent turn ends.
...
async def main(wav_path: str):
    ...
    async with websockets.connect(URL, max_size=None) as ws:
        ...
        while True:
            ...
            elif ev["type"] in ("agent.stop_talking", "session.closed"):
                break
