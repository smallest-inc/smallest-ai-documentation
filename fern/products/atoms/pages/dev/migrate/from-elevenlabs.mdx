...
description: "Map ElevenLabs Conversational AI auth, agent creation, and WebSocket protocol to Smallest Atoms equivalents."
---
...
| Voice (`voice_id`) | Voice (`voiceId`, from the Waves catalog or a clone) |
...
| WebSocket, public agent | `?agent_id=<id>` (no token) |
| WebSocket, private agent | `?agent_id=<id>&token=<signed_token>`. Get the signed token from `GET /v1/convai/conversation/get-signed-url?agent_id=<id>`. |

**Smallest Atoms**

| Surface | Credential |
|---|---|
| REST | `Authorization: Bearer sk_<your-key>` header |
| WebSocket | `Authorization: Bearer sk_<your-key>` header, or `?token=sk_<your-key>` query param. Same flow for every agent. |

Create a Smallest API key in the [API Keys](https://app.smallest.ai/dashboard/api-keys) tab.
...
const agent = await elevenlabs.conversationalAi.agents.create({
  ...
  conversationConfig: {
    ...
    agent: { firstMessage: "Hello.", prompt: { prompt: "You are helpful." } },
  },
});
```
...
Agent creation uses three REST calls. The first call auto-publishes `V1` with default config. Subsequent config changes go through the draft + publish flow.
...
# 2. Create a draft off the active version
...
# 3. Patch the draft with prompt, voice, and LLM
...
Each published draft becomes an immutable version. See [Agent Versioning](/atoms/atoms-platform/features/versioning) for rollback and version comparison.
...
| `agent_response_correction` | Not emitted. Smallest does not publish mid-turn corrections. |
...
| `agent_chat_response_part` (text deltas) | Not emitted. Smallest streams audio, not text deltas. |
...
## 6. Python example (raw WebSocket)
...
Replace `API_KEY` and `AGENT_ID` with your own values. The script writes the agent's reply audio to `reply.wav`.

## 7. Browser SDK example
...
The SDK runs in browser environments only. It calls `navigator.mediaDevices.getUserMedia` and the Web Audio API, which are not available in React Native, Node, or mobile runtimes. For non-browser runtimes, use the raw WebSocket protocol from section 6.

Full API surface: [Browser SDK](/atoms/developer-guide/integrate/browser-sdk).

## 8. Unsupported in Smallest Atoms
...
## 10. Voices and cloning

ElevenLabs voice IDs (for example `aMSt68OGf4xUZAnLpTU8`) do not port. Smallest uses the [Waves voice catalog](/waves/documentation/getting-started/models), which has separate pools for `waves_lightning_v2`, `waves_lightning_v3_1`, and `waves_lightning_large`. To use a custom voice, [clone one](/waves/documentation/voice-cloning/instant-clone-ui) from a short audio sample and reference the resulting `voice_…` ID.
...
<CardGroup cols={2}>
  ...
  <Card title="Post-Call Metrics" icon="chart-bar" href="/atoms/atoms-platform/features/post-call-metrics">
    Disposition metrics extracted from call transcripts.
  </Card>
