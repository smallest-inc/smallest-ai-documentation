...
| `contextual_update` with `{ type, text }` | Not supported mid-session. Inject values before the session with [pre-call API variables](/voice-agents/build/agent-settings). |
...
Full protocol: [Realtime Agent API reference](/api-reference/voice-agents/realtime-agent/realtime-agent).
...
For the full API surface (configuration, methods, events, push-to-talk, text input, error handling, smoke test), see the [Agent Web SDK](/voice-agents/integrate/web-socket-sdk) reference.
...
ElevenLabs creates an agent with a single REST call. Smallest splits creation and configuration into four calls because config is versioned. See [Agent Versioning](/voice-agents/build/versioning) for the full model.
...
Full reference: [GET voices](/api-reference/models/text-to-speech/get-waves-voices). The [Voice Cloning dashboard](https://app.smallest.ai/dashboard/voice-cloning) also lists available voice IDs in a UI.

For a custom voice, [clone one](/models/voice-cloning/instant-clone-ui) from a short audio sample. Cloned voice IDs are prefixed with `voice_`.
...
|---|---|
| Prompt versioning (drafts, publish, activate, rollback, per-version test calls, metric comparison) | [Agent Versioning](/voice-agents/build/versioning) |
| Post-call disposition metrics extracted from the transcript, surfaced in the call log | [Post-Call Metrics](/voice-agents/build/post-call-metrics), [developer guide](/voice-agents/developer-guide/operate/analytics/post-call-analytics) |
...
<CardGroup cols={3}>
  <Card title="Realtime Agent WebSocket reference" icon="plug" href="/api-reference/voice-agents/realtime-agent/realtime-agent">
    Full wire protocol with message types, payloads, and error codes.
  </Card>
  <Card title="Agent Versioning" icon="code-branch" href="/voice-agents/build/versioning">
