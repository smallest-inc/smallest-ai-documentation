...
| `contextual_update` with `{ type, text }` | Not supported mid-session. Inject values before the session with [pre-call API variables](/atoms/atoms-platform/create-agent/agent-config). |
...
For the full API surface (configuration, methods, events, push-to-talk, text input, error handling, smoke test), see the [Agent Web SDK](/atoms/atoms-platform/integrate/web-socket-sdk) reference.
