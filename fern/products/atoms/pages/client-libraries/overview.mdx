...
`pip install smallestai` also installs the `smallestai` CLI. As of `smallestai>=5.4.2` it covers seven command groups. Every subcommand honors the same auth resolution: `SMALLEST_API_KEY` first, then `~/.smallestai/credentials.json` written by `smallestai auth login`.
