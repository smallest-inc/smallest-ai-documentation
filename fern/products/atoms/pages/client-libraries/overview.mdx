...
client.on("agent.start_talking", () => {
  console.log("Agent started talking");
});

client.on("agent.stop_talking", () => {
...
- `agent.start_talking` - Agent began speaking (voice mode)
- `agent.stop_talking` - Agent stopped speaking (voice mode)
...
class AtomsChat {
  ...
  private setupEventListeners() {
    ...
    if (this.mode === "webcall") {
      this.client.on("agent.start_talking", () => {
        this.showAgentSpeaking(true);
      });

      this.client.on("agent.stop_talking", () => {
