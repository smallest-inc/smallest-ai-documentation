...
client = SmallestAI()
try:
    client.waves.synthesize_tts(text="hello", voice_id="avery")
...
| `smallestai agent-crew` | Crew project scaffold, deploy, and chat against a running crew. |
| `smallestai calls` | Start outbound calls and inspect call records. |
| `smallestai waves` | TTS synthesis, STT transcription, voice catalog lookup, voice cloning. |
