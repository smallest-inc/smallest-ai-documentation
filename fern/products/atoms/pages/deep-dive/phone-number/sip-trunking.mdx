...
<Frame caption="SIP Trunks page with the empty state. Inbound and outbound are separate resources.">
  ![SIP Trunks empty state](./images/sip-trunks-empty.png)
</Frame>

The inbound-trunk dialog has two shapes selected by a **Phone numbers / IP address** tab:

<Frame caption="Inbound trunk, Phone numbers shape. Dedicated trunk with its own credentials, media encryption, and Krisp toggle.">
  ![Inbound trunk, Phone numbers tab](./images/inbound-trunk-phone-numbers-tab.png)
</Frame>

<Frame caption="Inbound trunk, IP address shape. Only the allow-list is settable; the response returns the number your SBC dials.">
  ![Inbound trunk, IP address tab](./images/inbound-trunk-ip-address-tab.png)
</Frame>

The outbound-trunk dialog:

<Frame caption="Outbound trunk. Caller IDs the trunk may present, transport, media encryption, and per-carrier CPS ceiling.">
  ![Outbound trunk dialog](./images/outbound-trunk-dialog.png)
</Frame>
...
- **Auth**: digest username/password (both or neither), or leave blank for IP-based auth. For inbound, `allowedAddresses` restricts which hosts may send you calls. An inbound trunk on the **IP address** shape (no numbers, no allowed caller numbers) answers any extension from those peers and returns a `sipIdentity` + `sipHost` in the response; see [Direct SIP-to-SIP (IP-address trunks)](#direct-sip-to-sip-ip-address-trunks) below.
...
<Frame caption="Agent Telephony tab: Answers on, Dials from, Transfer from. Each section maps directly to one API resource.">
  ![Agent Telephony tab](./images/agent-phone-numbers-tab.png)
</Frame>
...
## Direct SIP-to-SIP (IP-address trunks)

If your carrier or SBC does not present phone numbers on the wire (a direct SIP endpoint, an internal PBX, or a carrier that only knows how to dial a URI), create an inbound trunk with **no numbers and no allowed caller numbers**. The response returns a minted `sipIdentity` (an `+8835`-range E.164 number) and a `sipHost`.

```bash
curl -X POST .../v1/sip-trunk/inbound \
  -d '{ "name": "Acme direct SIP", "allowedAddresses": ["203.0.113.10"] }'
# 201:
# {
#   "trunkKind": "ipAddress",
#   "sipIdentity": "+883512345678901",
#   "sipHost": "your-project.sip.livekit.cloud",
#   ...
# }
```

Configure your SBC to dial `sipIdentity@sipHost` (append `;transport=tls` on 5061 or `;transport=tcp` on 5060 as usual). Calls route to whichever agent answers on this trunk.

Because the shape rides the shared platform ingress, trunk-level fields (`authUsername`, `authPassword`, `mediaEncryption`, `krispEnabled`, `ringingTimeoutSeconds`) are refused with a 400 explaining why. Add a phone number or an `allowedNumbers` entry if you need a dedicated trunk with those settings.
