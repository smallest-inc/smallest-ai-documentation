...
4. **Numbers in E.164 format** with the leading `+` (`+919240923803`, not `919240923803`). Our ingress requires the `+` on the request URI. Without it, the call routes as if the destination were unmapped.

## Transport contract

**This is the section that costs people days when it is missed.** Our SIP ingress accepts SIP signalling on TCP and TLS only. UDP INVITEs are silently dropped; you get no response, not even a rejection. Every retransmission times out.
...
```
sip:+919240923803@5rdbhjuwtlk.sip.livekit.cloud;transport=tls   ← preferred, port 5061
sip:+919240923803@5rdbhjuwtlk.sip.livekit.cloud;transport=tcp   ← fallback, port 5060
```
...
- **Destination URI**: `sip:+<E.164>@5rdbhjuwtlk.sip.livekit.cloud;transport=tls`
...
2. Make an **outbound** test call from the agent (via the dashboard, or [`POST /conversation/outbound`](/voice-agents/developer-guide/build/phone-calling/outbound-calls)) to your own phone. Check that your provider bills the call to your termination trunk.
...
```
sip:+<E.164>@5rdbhjuwtlk.sip.livekit.cloud;transport=tls
```
...
**Fix**: add `;transport=tcp` (port 5060) or `;transport=tls` (port 5061) to the request URI, or configure the transport in your trunk's SIP settings. That single change is the fix for the vast majority of "no response" reports.

Confirm with a `sipsak` OPTIONS ping after the change:
...
- **Missing `+` prefix.** Our ingress requires the leading `+` on the request URI. A `To: sip:919240923803@...` INVITE reaches us but does not match a mapped number, so the ingress does not route it to an agent. Rewrite your SBC to always send `sip:+919240923803@...`.
...
<AccordionGroup>
  <Accordion title="Adding TCP transport" icon="wrench">
    ...
    Open port 5060 TCP inbound in your security group. Reload Kamailio and re-send INVITEs to `sip:+<E.164>@5rdbhjuwtlk.sip.livekit.cloud;transport=tcp`.
  ...
  <Accordion title="Adding TLS transport" icon="lock">
    ...
    Configure `tls.cfg` with your certificate chain and re-send INVITEs to `sip:+<E.164>@5rdbhjuwtlk.sip.livekit.cloud;transport=tls`.
...
<AccordionGroup>
  ...
  <Accordion title="Own SBC (Asterisk, FreeSWITCH, Kamailio)" icon="server">
    Configure your outbound trunk to `5rdbhjuwtlk.sip.livekit.cloud` with `transport=tls` on 5061 (or `transport=tcp` on 5060 as a fallback). Do not use UDP. Set the request URI to `sip:+<E.164>@5rdbhjuwtlk.sip.livekit.cloud;transport=tls`.
