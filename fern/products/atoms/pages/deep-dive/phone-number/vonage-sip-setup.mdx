...
2. Add Smallest AI's SIP endpoint as a destination, with **TLS transport** (preferred; TCP is a fallback):
   ...
   ```
   Our SIP ingress accepts TCP (5060) and TLS (5061) only. UDP INVITEs are silently dropped with no rejection, and every retransmission times out. See [SIP Trunking → Transport contract](/voice-agents/telephony/phone-numbers/sip-trunking#transport-contract).
...
- [SIP Trunking Overview](/voice-agents/telephony/phone-numbers) - General SIP trunking concepts
- [Phone Numbers](/voice-agents/telephony/phone-numbers) - Managing phone numbers on Smallest AI
