...
2. Add Smallest AI's SIP endpoint as a destination, with **TLS transport** (preferred; TCP is a fallback):
   ```
   sip:YOUR_SIP_INGRESS.sip.smallest.ai;transport=tls
   ```
   Use TLS (5061) or TCP (5060). UDP is accepted but not recommended: full-size INVITEs can fragment and be lost silently, which shows up as calls timing out with no response. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract).
3. Configure the SIP connection settings:
   - **Transport**: TLS (preferred) or TCP. Avoid UDP for signalling.
...
[
  {
    ...
    "endpoint": [
      {
        "type": "sip",
        "uri": "sip:YOUR_SIP_INGRESS.sip.smallest.ai;transport=tls"
      }
    ]
  }
]
...
2. Your Vonage outbound SIP URI typically follows this format:
   - `sip.nexmo.com`
   - Or a region-specific endpoint like `sip-us.nexmo.com` (hostname only - a `sip:` prefix is accepted but automatically stripped)
...
2. Enter these in the dedicated **Username** and **Password** fields when importing the number:
   - SIP Termination URL: `sip.nexmo.com` · Username: your API key · Password: your API secret
   - Do **not** embed credentials in the URL itself (`;username=...;password=...` parameters are stripped during normalization)
...
4. Fill in your number details:
   ...
   - **SIP Termination URL**: Your Vonage SIP endpoint
     ...
     - Example: `sip.nexmo.com`
     - Or region-specific: `sip-us.nexmo.com`
     - Provide authentication via the **Username** / **Password** fields if required
...
- Check that SIP endpoint URI is correct and carries the transport parameter: `sip:YOUR_SIP_INGRESS.sip.smallest.ai;transport=tls`
...
- [SIP Trunking Overview](/voice-agents/platform/deploy/phone-numbers) - General SIP trunking concepts
- [Phone Numbers](/voice-agents/platform/deploy/phone-numbers) - Managing phone numbers on Smallest AI
