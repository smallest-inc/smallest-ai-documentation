...
2. Add Smallest AI's SIP endpoint as a destination, with **TLS transport** (preferred; TCP is a fallback):
   ...
   ```
   Our SIP ingress accepts TCP (5060) and TLS (5061) only. UDP INVITEs are silently dropped with no rejection, and every retransmission times out. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract).
3. Configure the SIP connection settings:
   - **Transport**: TLS (preferred) or TCP. **Do not use UDP.**
