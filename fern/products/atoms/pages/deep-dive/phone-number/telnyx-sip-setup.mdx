...
3. Add a new SIP URI destination pointing to Smallest AI, with **TLS transport** (preferred; TCP is a fallback):
   ```
   sip:YOUR_SIP_INGRESS.sip.smallest.ai;transport=tls
   ```
   Use TLS (5061) or TCP (5060). UDP is accepted but not recommended: full-size INVITEs can fragment and be lost silently. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract).
...
4. Fill in your number details:
   ...
   - **SIP Termination URL**: Your Outbound SIP URI from Step 3
     ...
     - Example: `sip.telnyx.com` (hostname only - a `sip:` prefix is accepted but automatically stripped)
...
- Verify the SIP URI in Telnyx points to: `sip:YOUR_SIP_INGRESS.sip.smallest.ai`
...
- Use the exact host provided by Telnyx (e.g., `sip.telnyx.com`)
