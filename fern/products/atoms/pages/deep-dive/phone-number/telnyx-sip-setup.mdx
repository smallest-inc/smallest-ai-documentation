...
3. Add a new SIP URI destination pointing to Smallest AI, with **TLS transport** (preferred; TCP is a fallback):
   ...
   ```
   Our SIP ingress accepts TCP (5060) and TLS (5061) only. UDP INVITEs are silently dropped with no rejection. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract).
