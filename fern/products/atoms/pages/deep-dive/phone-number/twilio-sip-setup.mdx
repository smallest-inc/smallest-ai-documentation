...
3. Enter Smallest AI's SIP server with **TLS transport** (preferred; TCP is a fallback):
   ```
   sip:YOUR_SIP_INGRESS.sip.smallest.ai;transport=tls
   ```
   Use TLS (5061) or TCP (5060). UDP is accepted but not recommended: full-size INVITEs can fragment and be lost silently. Full explanation on [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract).
...
4. Fill in your number details:
   ...
   - **SIP Termination URL**: Your Termination SIP URI from Step 2
     ...
     - Example: `mycompany.pstn.twilio.com` (hostname only - a `sip:` prefix is accepted but automatically stripped)
...
- Use the bare hostname format: `mycompany.pstn.twilio.com`
