...
3. Enter Smallest AI's SIP server with **TLS transport** (preferred; TCP is a fallback):
   ...
   ```
   Our SIP ingress accepts TCP (5060) and TLS (5061) only. UDP INVITEs are silently dropped. Full explanation on [SIP Trunking → Transport contract](/voice-agents/telephony/phone-numbers/sip-trunking#transport-contract).
