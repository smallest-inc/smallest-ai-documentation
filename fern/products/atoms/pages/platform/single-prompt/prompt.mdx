...
- **Tools.** For each tool you enable in [Agent Config](/voice-agents/build/agent-settings), when should the agent use it?

<Tip>
**Product Knowledge vs. Knowledge Base.** Small, stable facts, like what your product does or how your pricing works, belong directly in the prompt so the agent always has them on hand. Large or frequently changing content, like manuals or help docs, belongs in a [Knowledge Base](/voice-agents/build/agent-settings#knowledge-base) instead.
</Tip>
...
Variables get replaced with real values at runtime. See [Agent Config](/voice-agents/build/agent-settings#variables) to create and manage them.
