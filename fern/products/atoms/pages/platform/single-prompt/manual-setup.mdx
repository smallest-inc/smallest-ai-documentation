---
title: "Manual Setup"
sidebarTitle: "Start from Scratch"
description: "Build a Single Prompt agent from a blank canvas."
---

Starting from scratch gives you complete control. You'll land in an empty editor ready for your instructions.

---

## Getting There

<Steps>
  <Step title="Open Create Agent">
    From your dashboard, click the green **Create Agent** button in the top right.
  </Step>
  
  <Step title="Choose Start from Scratch">
    Select the first option in the modal.
    
    <Frame caption="Creation method selection">
      <img src="../building-agents/images/create-agent-modal.png" alt="Create agent modal" />
    </Frame>
  </Step>
  
  <Step title="Select Single Prompt">
    Choose **Single Prompt** as your agent type.
  </Step>
</Steps>

The editor opens with everything empty — ready for you to configure.

---

## What's Next

You're now in the editor. Here's where to go from here:

<CardGroup cols={2}>
  <Card title="Write Your Prompt" icon="pen" href="/platform/single-prompt/writing-prompts">
    The heart of your agent — craft effective instructions
  </Card>
  <Card title="Pick Your Model" icon="microchip" href="/platform/single-prompt/config/model-selection">
    Choose the LLM that powers your agent
  </Card>
  <Card title="Select a Voice" icon="waveform" href="/platform/single-prompt/config/voice-selection">
    Give your agent a voice that fits
  </Card>
  <Card title="Configure Features" icon="sliders" href="/platform/single-prompt/config/end-call">
    End call, transfer, knowledge base, and more
  </Card>
</CardGroup>

<Tip>
**First time?** Start by writing your prompt, picking a voice, and hitting **Test Agent** (top right). You can refine everything else as you go.
</Tip>
