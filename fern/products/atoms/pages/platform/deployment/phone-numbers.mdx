...
<Tabs>
  ...
  <Tab title="Import SIP">
    ...
    | **SIP Origination URL** | - | Provided by the platform (copy to your provider). Append `;transport=tls` (preferred) or `;transport=tcp` before pasting; our ingress does not accept UDP. See [SIP Trunking → Transport contract](/voice-agents/telephony/phone-numbers/sip-trunking#transport-contract). |
...
Configure these on the agent under **Agent Config**. See [Agent Config](/voice-agents/build/agent-settings) for the End Call and Transfer Call settings.
...
<CardGroup cols={2}>
  <Card title="Audiences" icon="users" href="/voice-agents/telephony/audiences">
    Manage contact lists for campaigns
  </Card>
  <Card title="Campaigns" icon="bullhorn" href="/voice-agents/telephony/campaigns">
