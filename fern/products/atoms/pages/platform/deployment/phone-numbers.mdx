...
<Tabs>
  ...
  <Tab title="Import SIP">
    ...
    | **SIP Origination URL** | - | Provided by the platform (copy to your provider). Append `;transport=tls` (preferred) or `;transport=tcp` before pasting; our ingress does not accept UDP. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract). |
