...
Get any of these wrong and the PCA model's output fails validation for that call — so no metric at all is stored for it, not just the one that broke. The five rules below cover ~all of the failure patterns we see in production.
...
<Steps>
  ...
  **Common failure:** an integer `promised_amount` field with a fallback like *"output 'Not Applicable' if no amount was promised"* — the string fails the integer type check.
  ...
  Unspecified weights make the model invent its own, so the same call can score differently across runs — non-reproducible.
</Steps>
