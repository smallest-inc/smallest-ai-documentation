        ...
        "postCallAnalyticsConfig": {
            "dispositionMetrics": [
                ...
                },
                {
                    ...
                    "dispositionValues": {"type": "INTEGER"},
                },
                {
                    "identifier": "call_outcome",
...
| `dispositionValues.type` | Yes | One of `STRING`, `BOOLEAN`, `INTEGER`, `ENUM`, `DATETIME`. |
...
The active version's metrics are surfaced under `_resolvedConfig.postCallAnalyticsConfig`. After publishing, the backend stores `dispositionMetricType` (flat) in place of the `dispositionValues.type` shape sent in the request.
...
<CardGroup cols={2}>
  <Card title="Conversation Logs" icon="scroll" href="/atoms/atoms-platform/analytics-logs/conversation-logs">
    View metrics for individual calls
  </Card>
  <Card title="Analytics Dashboard" icon="chart-line" href="/atoms/atoms-platform/analytics-logs/overview">
