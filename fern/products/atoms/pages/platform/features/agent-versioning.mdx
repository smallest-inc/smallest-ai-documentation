...
subtitle: "Prompt, Tools, Voice, Language, Post call metrics, and more are managed as **versions**. You edit in isolated drafts, publish drafts as new versions, and activate the version you want live."
description: "Edit agent config safely with drafts, published versions, and rollback."
slug: /voice-agents/platform/features/versioning
---
...
<Frame caption="Read-only view. Editing requires a draft.">
  ![Create a draft to edit](../building-agents/images/agent-versioning/01-create-draft-modal.png)
</Frame>
...
<Frame caption="Fork a new draft from an existing draft.">
  ![New draft from draft](../building-agents/images/agent-versioning/08-drafts-new-draft-hover.png)
</Frame>
...
<Frame caption="Draft #2-ce62 is now editable. Edits auto-save with a revision counter.">
  ![Draft editable](../building-agents/images/agent-versioning/02-editing-draft.png)
</Frame>
...
<Tabs>
  <Tab title="Version history">
    ...
    <Frame caption="Version history. V1 is active.">
      ![Version history](../building-agents/images/agent-versioning/03-version-history.png)
    </Frame>
  </Tab>
  <Tab title="Drafts">
    ...
    <Frame caption="Two drafts, both forked from V1.">
      ![Drafts panel](../building-agents/images/agent-versioning/04-drafts-panel.png)
    </Frame>
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Pick a base and a comparison">
    ...
    <Frame>
      ![Select versions to compare](../building-agents/images/agent-versioning/05-compare-versions-select.png)
    </Frame>
  </Step>
  <Step title="Review the diff">
    ...
    <Frame>
      ![Prompt diff](../building-agents/images/agent-versioning/06-compare-versions-prompt-diff.png)
    </Frame>
  </Step>
  <Step title="Inspect config sections">
    LLM model, voice config, and language settings each render as their own grouped diff:
    <Frame>
      ![Config diff](../building-agents/images/agent-versioning/07-compare-versions-config-diff.png)
    </Frame>
  </Step>
</Steps>
...
<Frame caption="Review diff, label the version, and choose whether to activate on publish.">
  ![Review Changes](../building-agents/images/agent-versioning/09-review-changes-publish.png)
</Frame>
...
<Frame caption="V2 is now active. V1 can still be restored.">
  ![After publish](../building-agents/images/agent-versioning/10-after-publish-v2-active.png)
</Frame>
...
requests.patch(
    ...
    json={
        ...
        "synthesizer": {
            "voiceConfig": {"model": "waves_lightning_v3_1", "voiceId": "nyah"},
            "speed": 1.2,
        },
    },
).raise_for_status()

# 4. Publish the draft as a new version
published = requests.post(
    f"{BASE}/agent/{AGENT_ID}/drafts/{draft_id}/publish",
    headers=HEADERS,
    json={"label": "v2 - friendlier prompt", "description": "Softened tone"},
).json()
version_id = published["data"]["_id"]

# 5. Activate the new version (publish does NOT auto-activate, even with `activate: True`)
# A security check runs immediately after publish; wait ~6 s for it to complete.
import time
time.sleep(6)
requests.patch(
...
print("New active version:", version_id)
```
...
| Publish a draft | `POST /agent/{id}/drafts/{draftId}/publish` (creates a new version; does **not** activate — call activate explicitly) |
...
| Activate a version | `PATCH /agent/{id}/versions/{versionId}/activate` (security check runs immediately after publish; wait ~6 s if you just published) |
| Update version metadata | `PATCH /agent/{id}/versions/{versionId}` (label, description, isPinned) |
| Archive (delete) an agent | `DELETE /agent/{id}/archive` (note: `/archive` suffix; plain `DELETE /agent/{id}` returns 404) |
...
<Card title="Versioning Lifecycle (Developer Guide)" href="/atoms/developer-guide/build/agent-crews/versioning-lifecycle">
  Same flow in curl + Python with copy-pasteable snippets.
</Card>
<CardGroup cols={2}>
  ...
  <Card title="API Reference" href="/atoms/api-reference">
