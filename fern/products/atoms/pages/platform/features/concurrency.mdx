...
<Note>
  Concurrency is how many calls run **at once**. How fast new outbound calls **start** is a separate limit, see [Calls per second (CPS)](/voice-agents/platform/create-agent/cps). Both apply together.
</Note>
...
<Warning>
  **Set the reservation before you start a campaign.** Campaigns dial out at a fixed rate. If the shared pool is already saturated when the campaign fires, the campaign's outbound calls fail for lack of slots. Reserving concurrency for the campaign's agent up front guarantees it a floor. Changes made after the campaign starts apply to new dial attempts only, they cannot rescue calls that already failed.
</Warning>
...
<AccordionGroup>
  <Accordion title="Do I need to reserve concurrency before running a campaign?">
    **Yes, set the reservation on the campaign's agent before the campaign starts.** Campaigns dial out at a fixed rate; if the org's shared pool is already saturated when the campaign fires, the campaign's outbound calls fail because there are no slots left for them. Reserving guarantees the campaign's agent at least its floor regardless of what other agents are doing.

    Adjusting the reservation while the campaign is already running takes effect on new dial attempts only. It cannot rescue calls that already failed for lack of slots.
  </Accordion>
