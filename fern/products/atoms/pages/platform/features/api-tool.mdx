...
An **API tool** calls your endpoint during the conversation and feeds the response back to the agent. Build it once in the [Tools library](/voice-agents/build/tools/tools-library) (or from inside an agent) and reference it anywhere.
...
The **Auth** tab handles authentication. You never paste a raw key here — you reference a [secret](/voice-agents/build/tools/secrets) by name, and the platform injects it into the outbound request at call time.
...
<CardGroup cols={2}>
  <Card title="Tools library" href="/voice-agents/build/tools/tools-library">
    Create once, reference across agents
  </Card>
  <Card title="Secrets" href="/voice-agents/build/tools/secrets">
