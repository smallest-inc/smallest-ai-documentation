...
<Warning>
A referenced (imported) tool is shared state. Editing it here edits the one library record, so the change also lands on every other agent that references it — not just this agent. To diverge, [duplicate](/voice-agents/platform/create-agent/tools-library#one-tool-one-shared-state) it instead.
</Warning>
