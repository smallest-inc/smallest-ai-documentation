...
<Note>
Custom API and client tools are now **reusable across agents** — build one in the [Tools library](/voice-agents/build/tools/tools-library) and reference it anywhere. This page covers the per-agent Tools tab and built-in System tools.
</Note>
...
For anything not covered by a system tool, connect your own API or app logic. Click **Add tool** and choose a type. Creating one here adds it to the [Tools library](/voice-agents/build/tools/tools-library) and references it on this agent, so you can reuse it elsewhere. Full builders: [API tool](/voice-agents/build/tools/api-tool) and [Client tools](/voice-agents/build/tools/client-tools).

<Warning>
A referenced (imported) tool is shared state. Editing it here edits the one library record, so the change also lands on every other agent that references it — not just this agent. To diverge, [duplicate](/voice-agents/build/tools/tools-library#one-tool-one-shared-state) it instead.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Prompt" href="/voice-agents/build/prompt">
