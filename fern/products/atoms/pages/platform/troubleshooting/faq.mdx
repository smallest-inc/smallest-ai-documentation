...
The `To:` header on your INVITE must carry the number in E.164 with the leading `+` (`sip:+919240923803@...`, not `sip:919240923803@...`). Without the `+` the ingress does not map the request to your imported number. Rewrite your SBC's request URI to always send `+<country_code><number>`. Full checklist under [SIP Trunking → Troubleshooting](/voice-agents/platform/deploy/phone-numbers/sip-trunking#troubleshooting).
