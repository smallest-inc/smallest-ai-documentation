...
Almost always the transport. Over UDP, a full-size INVITE (SDP body plus headers) can fragment, and fragments are silently discarded by many networks: no reply, no rejection, retransmissions all time out, while a small `OPTIONS` ping over the same UDP path works fine. Append `;transport=tls` (port 5061, preferred) or `;transport=tcp` (port 5060) to the origination URL you configure at your provider. See [SIP Trunking → Transport contract](/voice-agents/platform/deploy/phone-numbers/sip-trunking#transport-contract) for the exact URL shape and a `sipsak` OPTIONS ping you can run to confirm reachability.

### My SIP INVITE reaches you but the agent never picks up. What am I missing?

The `To:` header on your INVITE must carry the number in E.164 with the leading `+` (`sip:+91XXXXXXXXXX@...`, not `sip:91XXXXXXXXXX@...`). Without the `+` the ingress does not map the request to your imported number. Rewrite your SBC's request URI to always send the country code and number together with a leading `+`. Full checklist under [SIP Trunking → Troubleshooting](/voice-agents/platform/deploy/phone-numbers/sip-trunking#troubleshooting).
...
<CardGroup cols={2}>
  <Card title="Glossary" icon="book" href="/voice-agents/platform/reference/glossary">
    Key terms defined
  </Card>
  <Card title="Getting Help" icon="life-ring" href="/voice-agents/platform/troubleshooting/getting-help">
    Contact support
  </Card>
</CardGroup>
