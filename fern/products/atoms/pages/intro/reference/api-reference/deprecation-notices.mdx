...
slug: /voice-agents/api-reference/deprecation-notices
---
...
| `PATCH /workflow/{workflowId}` (direct workflow graph edits) | 2026-06-11 | Rejects with `400` | Same as above &mdash; edit the `workflow` field on a draft, then publish. |

### Agent Versioning &mdash; Drafts (v1)
...
### Agent Versioning &mdash; Versions (v1)
...
| `GET /agent/{id}/versions/{versionId}/compare-metrics` | Deprecated (still works) | No change |
...
Full migration walkthrough with worked examples: [v1 → v2 migration guide](/voice-agents/api-reference/versioning-v2-migration).
