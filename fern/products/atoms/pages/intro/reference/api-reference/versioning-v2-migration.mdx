---
title: "Migrating from v1 to the branch/revision API"
...
slug: /voice-agents/api-reference/versioning-v2-migration
---
...
- **~1 month coexistence**: kept v1 endpoints continue to work so by-id reads and test-calls do not break.
- **Sunset**: v1 endpoints are removed. Date is not yet fixed. This page and the [Deprecation Notices](/voice-agents/api-reference/deprecation-notices) page will carry the final sunset date once it lands, along with the `Sunset:` header on the affected endpoints.
...
```json
{
  ...
  "errors": ["This endpoint is deprecated on the v2 branch model. See the migration guide for the v2 equivalent."]
}
```
