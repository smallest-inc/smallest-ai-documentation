...
Splitting the trunk into two resources is also what enables direct SIP-to-SIP. An inbound trunk comes in two shapes:

- **Dedicated (`trunkKind: phoneNumber`).** Send `numbers` or `allowedNumbers`; the trunk gets its own credentials, media-encryption policy, Krisp toggle, and ringing timeout.
- **IP-address (`trunkKind: ipAddress`).** Send just `name` and `allowedAddresses`. The response includes a minted `sipIdentity` (an `+8835`-range E.164 number) and a `sipHost`. Configure your SBC to dial `sipIdentity@sipHost`. Trunk-level fields (auth, encryption, Krisp, timeout) are refused on this shape because it rides the shared platform trunk and has no per-tenant settings.

Both shapes were impossible with the old single import, which required a phone number.
