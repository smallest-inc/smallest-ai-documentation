...
Two new fields on [`POST /conversation/register-call`](/voice-agents/api-reference/realtime-agent/register-call) and on the WebSocket connect URL:
...
The accepted tokens and error bodies are on [Register Call](/voice-agents/api-reference/realtime-agent/register-call) and [Agent WebSocket](/voice-agents/api-reference/realtime-agent/realtime-agent). Opus framing, the voice-dependent output rate, and migrating off `sample_rate` are on the new [Audio Formats](/voice-agents/platform/agent-sdk/audio-formats) page.
