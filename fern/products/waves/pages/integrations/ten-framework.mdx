...
Any additional key in `params` (e.g. `word_timestamps`, `eou_timeout`, `punctuate`) is forwarded verbatim as a query parameter to the live endpoint - see the [Pulse feature docs](https://docs.smallest.ai/models/speech-to-text/overview) for the full list.
...
Any additional key in `params` (e.g. `language`, `pronunciation_dicts`) is forwarded verbatim in the request body - see the [TTS API reference](https://docs.smallest.ai/api-reference/models/text-to-speech/synthesize-speech).
