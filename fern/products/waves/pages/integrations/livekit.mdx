...
| `keywords` | `list[tuple[str, float]]` | `[]` | Boost recognition via `(keyword, intensifier)` pairs, e.g. `[("NVIDIA", 2.0)]`. Intensifier ~1.0 (mild) to ~5.0 (strong); avoid values above 5 (hallucination risk). Streaming only, up to 100 keywords per session |
