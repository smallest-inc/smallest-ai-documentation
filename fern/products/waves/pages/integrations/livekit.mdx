...
# Automatic language detection across Indian languages (streaming)
stt = smallestai.STT(language="north_indic")
...
| `model` | `str` | `"pulse"` | STT model. `"pulse"` — streaming + batch, 38 languages. `"pulse-pro"` — higher-accuracy English-only, batch (pre-recorded) only; raises `ValueError` if used with streaming |
| `language` | `str` | `"en"` | Language code (e.g. `"en"`, `"hi"`, `"fr"`). LiveKit uses Pulse's streaming surface. Use `"north_indic"` (Indian-language auto-detect), `"multi-asian"` (East Asian auto-detect, US region only), or `"multi-south-indic"` (South Indian auto-detect, India region only, Beta) when the language is unknown. See the [Pulse model card](/models/model-cards/speech-to-text/pulse) for the full streaming code list |
...
| `eou_timeout_ms` | `int` | `100` | Silence (ms) before the server finalizes an utterance. Range `100`–`10000`ms; defaults to `100`ms so server-side EOU adds minimal latency on top of LiveKit's own end-of-turn detection. Acts as a ceiling when `endpointing` is enabled; the sole finalization trigger when disabled |
| `endpointing` | `bool` | `True` | Finalize on trailing silence instead of waiting on `eou_timeout_ms` alone |
| `keywords` | `list[tuple[str, float]]` | `[]` | Boost recognition via `(keyword, intensifier)` pairs, e.g. `[("NVIDIA", 2.0)]`. Intensifier ~1.0 (mild) to ~5.0 (strong); avoid values above 5 (hallucination risk). Streaming only, up to 10,000 keywords per session |
| `format` | `bool` | `True` | Apply punctuation and capitalization to streaming transcripts |
| `sentence_timestamps` | `bool` | `False` | Include sentence-level timing in streaming transcripts as an `"utterances"` list in `SpeechData.metadata["utterances"]`. Streaming only |
| `redact_pii` | `bool` | `False` | Mask PII (names, addresses, phone numbers) with placeholder tokens; matches are also listed in `SpeechData.metadata["redacted_entities"]`. Reliable only for `"en"`/`"hi"`. Streaming only |
| `redact_pci` | `bool` | `False` | Mask payment card info (card numbers, CVVs, ZIP codes, account numbers) with placeholder tokens; matches are also listed in `SpeechData.metadata["redacted_entities"]`. Reliable only for `"en"`/`"hi"`. Streaming only |
...
| `language` | `str` | `"en"` | Language code. `lightning_v3.1` supports 12 codes plus `"auto"`; `lightning_v3.1_pro` supports `"en"`, `"hi"`, and `"auto"` only — see the [model cards](/models/model-cards/text-to-speech) for the full lists |
...
| `max_buffer_flush_ms` | `int` | `0` | Server-side buffer bound (ms) for continuous WebSocket streaming: once this many milliseconds of streamed text have buffered, the server force-flushes partial audio without waiting for an explicit flush. `0` disables time-based flushing (audio is produced on the segment's end-of-input flush). Raise it (e.g. 200–400) to trade a little latency for fewer, larger audio chunks. Streaming only. |
