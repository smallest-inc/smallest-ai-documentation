...
|---|---|
| `lightning_v3.1` | Default. 44.1 kHz, natural expressive speech, ~200 ms TTFB, 20 accepted language codes (12 with trained voices) plus `auto` for cross-language routing, voice cloning supported. See the [Lightning v3.1 model card](/models/model-cards/text-to-speech/lightning-v-3-1#supported-languages) for the full language list. |
| `lightning_v3.1_pro` | Premium broadcast-quality pool. 31 languages (adds 8 Asian & Middle Eastern + Greek/Finnish/Norwegian/Dutch/Swedish over base). American / British / Indian accents; Indian voices code-switch automatically. Supports voice cloning (`model: lightning-v3.1-pro`). See the [Lightning v3.1 Pro model card](/models/model-cards/text-to-speech/lightning-v-3-1-pro). |
...
The provider exposes the voice-cloning REST endpoints alongside TTS / STT. Cloning accepts `model: lightning-v3.1` (the default) or `lightning-v3.1-pro`; pair the resulting `voice_id` with the matching TTS model.
