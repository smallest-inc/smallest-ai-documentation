...
3. **Test Network Connectivity**:
   ...
   curl -i -N -H "Connection: Upgrade" \
        ...
        "https://api.smallest.ai/waves/v1/stt/live?model=pulse&language=en"
   ```
...
For 401, 403, 429, and 500 (including WebSocket handshake failures), see the [Error reference](/models/documentation/troubleshooting/error-reference). Browsers cannot set custom headers on WebSocket handshakes, so browser clients should authenticate via the `?token=<KEY>` query parameter where supported.
