...
Head over to the [smallest console](https://app.smallest.ai/dashboard/api-keys) to generate an API key, if not done previously. Also look at the [Authentication guide](/api-reference/authentication) for more information about API keys.
...
- Learn about [supported audio formats](/models/speech-to-text/pre-recorded/audio-formats).
- Decide which enrichment options to enable in the [features guide](/models/speech-to-text/pre-recorded/features).
- Configure asynchronous callbacks with [webhooks](/models/speech-to-text/pre-recorded/webhooks).
- Review a full [code example](/models/speech-to-text/pre-recorded/code-examples) here.
