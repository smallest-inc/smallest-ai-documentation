...
Pulse ships speaker diarization on both batch and streaming through the same API — no separate diarization vendor to stitch in. See the [Diarization](/models/documentation/speech-to-text-pulse/features/diarization) page for the response shape and enable flag.
...
- Batch DER numbers are from `elevenlabs_diarization_eval.md` (internal) run across 6 public datasets, 493 files, whole-file inference, default settings per vendor.
- Extended 7-system table (pyannoteAI, Deepgram, AssemblyAI, Speechmatics, Pulse, plus OSS baselines streaming-Sortformer / MAGO / NeMo-clustering) uses N=10 per dataset — smaller sample, use for orientation not for procurement decisions. See `diarization_eval_benchmark_only.md` (internal).
- Vendor numbers reflect defaults at time of test. Some vendors expose diarization-tuning knobs that were not enabled; results may differ under vendor-specific settings.
- Streaming Indic DER measured on internal telephony + read-speech corpora; production audio characteristics may vary.
