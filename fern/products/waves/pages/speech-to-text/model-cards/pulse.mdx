...
35 unique language codes across the two modes (21 on streaming, 26 on pre-recorded, 12 codes overlap). Click an accordion to expand the full per-mode list.
...
| 25 | Japanese | `ja` |
| 26 | Korean | `ko` |
| 27 | Multi-EU aggregator | `multi-eu` (auto-detects across the European codes above plus `en`). Pre-recorded only. |
| 28 | Multi-Asian aggregator | `multi-asian` (auto-detects across `zh`, `ko`, `ja`, `en`) |
| 29 | Multi-Indic aggregator | `multi-indic` (auto-detects across `en`, `hi`, `gu`, `mr`, `bn`, `or`). India region only. |
