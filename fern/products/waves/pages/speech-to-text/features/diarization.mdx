...
description: 'Attach zero-indexed speaker labels to each word and utterance in a transcript. Available on Pulse batch and streaming with the same response shape and the same enable flag.'
---
...
Available on Pulse in both batch (`POST /waves/v1/stt/`) and streaming (`WSS /waves/v1/stt/live`). Same enable flag. Same response shape. Not available on Pulse Pro — the Pro pool is optimised for English word accuracy and does not diarize; a request with `diarize=true` on `?model=pulse-pro` returns transcript-only with no speaker fields.
...
- `words[]` — per-word entries. Present when `word_timestamps=true` (batch) or on final events (streaming). Each entry gains `speaker` (integer) and `speaker_confidence` (0.0–1.0) when diarization is on.
- `utterances[]` — sentence-level entries. Present by default on batch; on streaming with `sentence_timestamps=true`. Each entry gains `speaker` (integer) when diarization is on.

Speaker labels are **integers, zero-indexed** (`0`, `1`, `2`, ...) on both batch and streaming. Labels are stable within a single response or session but not across separate requests: speaker `0` in one call is not necessarily the same physical speaker as speaker `0` in the next.
