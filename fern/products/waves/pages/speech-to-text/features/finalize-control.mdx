...
- [Inverse Text Normalization (ITN)](/models/speech-to-text/features/inverse-text-normalization) - `finalize_on_words` is most useful in combination with ITN.
- [End-of-Utterance Timeout](/models/speech-to-text/features/end-of-utterance-timeout) - controls automatic finalization timing when `finalize_on_words` stays `true`.
- [Pulse STT WebSocket reference](/api-reference/models/speech-to-text/speech-to-text) - full parameter reference and the `finalize` / `close_stream` control messages.
