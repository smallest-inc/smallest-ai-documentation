...
Our evaluation guide outlines a repeatable process: choose representative audio, generate transcripts, compute WER/CER/latency, and document findings. The ready-to-run snippets below call the [Pulse pre-recorded REST endpoint](/api-reference/models/speech-to-text/transcribe) directly so you can audit the methodology and adapt it to any HTTP client.
...
- **RTF** from `rtf` - values **below 1.0** mean Pulse processed audio faster than real-time. Pulse STT typically runs near 0.4 on clean inputs (see [Metrics Overview](/models/speech-to-text/benchmarks/metrics-overview#real-time-factor-rtf)).
...
- For proper-noun mistakes, queue them for a [pronunciation dictionary](/models/text-to-speech/pronunciation-dictionaries) if you also use Lightning TTS, or for the Pulse [keyword boosting](/models/speech-to-text/features/keyword-boosting) feature on the streaming endpoint.
