...
[`GET /waves/v1/{model}/get_voices`](/models/api-reference/text-to-speech/get-voices) now documents both Lightning v3.1 pools in the path enum:
...
For browsing every voice in one call across all models, keep using [`GET /waves/v1/voice/get-all-models`](/models/api-reference/text-to-speech/get-all-voices). For the canonical per-language voice list (with previews and recommended pairings), see the [Lightning v3.1](/models/model-cards/text-to-speech/lightning-v-3-1) and [Lightning v3.1 Pro](/models/model-cards/text-to-speech/lightning-v-3-1-pro) model cards.
