...
The `output_format` enum on Lightning v3.1 (`POST /waves/v1/lightning-v3.1/get_speech` and `/stream`) is now correctly documented as `["pcm", "mp3", "wav", "ulaw", "alaw"]`. The previously listed `mulaw` is rejected by the platform with an `invalid_enum_value` error and has been removed.
