...
description: "Generate speech synchronously or concurrently — REST API examples."
icon: "wave-square"
---
...
<Note>
The `smallestai` Python SDK's `WavesClient` and `AsyncWavesClient` synthesis methods are being updated. Use the `requests` / `aiohttp` examples above until the next SDK release. (Streaming synthesis via `WavesStreamingTTS` works — see [Streaming TTS](/models/documentation/text-to-speech-lightning/streaming).)
</Note>
...
| `voice_id` | string | *required* | Voice to use (e.g., `meher`, `magnus`, `olivia`, `aarush`) |
| `model` | string | `lightning_v3.1` | TTS pool to use. Pass `lightning_v3.1_pro` to route to the [Pro pool](/models/model-cards/text-to-speech/lightning-v-3-1-pro). |
| `sample_rate` | int | `44100` | `8000`, `16000`, `24000`, or `44100` Hz |
| `speed` | float | `1.0` | Speech rate multiplier (`0.5` to `2.0`) |
| `language` | string | `en` | Language code matching the voice. Indian: `en`, `hi`, `mr`, `kn`, `ta`, `bn`, `gu`, `te`, `ml`, `pa`, `or`. European: `es`. Each voice supports a subset — see voice tags via `GET /waves/v1/lightning-v3.1/get_voices`. |
| `output_format` | string | `pcm` | Audio format: `pcm`, `wav`, `mp3`, `ulaw`, or `alaw` |
| `pronunciation_dicts` | array | — | List of [pronunciation dictionary](/models/documentation/text-to-speech-lightning/pronunciation-dictionaries) IDs |
...
For real-time streaming where audio starts playing before generation completes, see [Streaming TTS](/models/documentation/text-to-speech-lightning/streaming).
