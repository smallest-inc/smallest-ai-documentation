...
<Tip>
Sending incremental text? Tag each fragment with the same `context_id` to have them buffered, joined at natural sentence boundaries, and spoken as one continuous generation instead of resetting prosody per chunk. See [Continuations](/models/text-to-speech/continuations).
</Tip>
...
- **Exception:** inside a [continuation](/models/text-to-speech/continuations) (a request carrying `context_id`), `complete` is *not* terminal — it marks one released segment as done, and you can get several `complete` frames on the same open connection before the context actually ends.
...
For concurrency limits and connection management, see [Concurrency and Limits](/api-reference/concurrency-and-limits).
