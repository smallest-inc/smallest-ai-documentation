...
The two transports emit different JSON shapes. These are the **canonical shapes**; match your parser to the transport you're using.
...
<Note>
**Porting a WebSocket client to SSE?** The two shapes are not interchangeable. WebSocket wraps every chunk in `{status, data.audio}`; SSE returns `{audio}` at the top level. A parser written for the WebSocket envelope will silently drop every SSE frame because `msg["data"]` is undefined. Same for the terminator: WebSocket signals completion with `status:"complete"`; SSE signals it with a bare `{"done": true}`.
</Note>

## Latency

TTFB is ~200 ms measured in-region (see the [Lightning v3.1 model card](/models/model-cards/text-to-speech/lightning-v-3-1#throughput-latency--pricing)). Client-to-server RTT adds on top. Measuring TTFB from a laptop far from `ap-south-1` or `us-west-2` typically reports 500-800 ms because RTT dominates synthesis. Test from the same network position where you'll run production traffic.
