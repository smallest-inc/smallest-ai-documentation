...
<Tip>
**Pairing rule.** Lightning v3.1 voice IDs only work with `"model": "lightning_v3.1"` (or with `model` omitted - the default). Lightning v3.1 Pro voice IDs only work with `"model": "lightning_v3.1_pro"`. Mismatched pairing returns synthesised audio in the wrong voice or no audio at all. See the [Pro model card](/models/model-cards/text-to-speech/lightning-v-3-1-pro) and [v3.1 model card](/models/model-cards/text-to-speech/lightning-v-3-1) for the full catalogs.
</Tip>
...
<Accordion title="European languages (61 voices + `nl`/`sv` pending - 12 languages)">
...
Dutch and Swedish accept the codes on Pro today; dedicated voice-count data is pending as new Pro voices for those roll out. Every multilingual Pro voice already synthesises both codes end-to-end.

</Accordion>

</AccordionGroup>

See the [Pro model card voice catalog](/models/model-cards/text-to-speech/lightning-v-3-1-pro#voice-catalog) for ranking notes.

## Lightning v3.1 - standard pool

217 voices total across 12 languages - English (176), Hindi (115), Tamil (13), Spanish (11), Kannada (10), Marathi (9), Telugu (8), Odia (8), Punjabi (8), Malayalam (6), Gujarati (5), Bengali (4). Voices below are the curated short-list; the [v3.1 model card](/models/model-cards/text-to-speech/lightning-v-3-1#voice-catalog) has the full catalog.
