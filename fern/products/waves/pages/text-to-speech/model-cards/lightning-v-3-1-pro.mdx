...
**Jump to:** [Benchmarks](#performance--benchmarks) · [Voice Catalog](#voice-catalog) · [API Reference](/api-reference/models/text-to-speech/synthesize-speech) · [Quickstart](/models/text-to-speech/quickstart)
...
See [Synthesize Speech](/api-reference/models/text-to-speech/synthesize-speech) for the full request/response schema, supported parameters, and error codes. To route to the Pro pool you must set `model` to `lightning_v3.1_pro` explicitly - the field is optional but defaults to standard Lightning v3.1.
...
Yes. Pass `model: lightning-v3.1-pro` to the [voice-cloning API](/models/voice-cloning/instant-clone-api) to clone onto the Pro pool, then use the resulting `voice_id` with TTS `model: lightning_v3.1_pro`. The default is `lightning-v3.1`.
</Accordion>
