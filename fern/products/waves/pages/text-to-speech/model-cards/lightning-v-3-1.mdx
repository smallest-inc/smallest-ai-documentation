...
description: "Model card for Lightning v3.1. High-fidelity, low-latency text-to-speech at 44 kHz with voice cloning, streaming, and 20 supported language codes plus `auto` routing."
icon: "sparkles"
---
...
**Jump to:** [Benchmarks](#performance--benchmarks) · [Voice Catalog](#voice-catalog) · [Supported Languages](#supported-languages) · [API Reference](/api-reference/models/text-to-speech/synthesize-speech) · [Pricing & Throughput](#throughput-latency--pricing) · [Quickstart](/models/text-to-speech/quickstart)
...
<CardGroup cols={4}>
  ...
  <Card title="20 Languages" icon="globe" href="#supported-languages">
...
<Note>
\*For Pronunciation and WER, the residual gap on Lightning v3.1 is concentrated in proper-noun rendering. Use a [pronunciation dictionary](/models/text-to-speech/pronunciation-dictionaries) to pin names, brands, and acronyms; with the dictionary applied, both metrics close to parity.
</Note>
...
Tag a sequence of WebSocket fragments with the same `context_id` to have them buffered, joined at natural sentence boundaries, and spoken as one continuous generation instead of resetting per-request - useful for text arriving incrementally (e.g. LLM token streams). WebSocket only; cannot be combined with `flush` or `max_buffer_flush_ms`. See [Continuations](/models/text-to-speech/continuations) for the full parameter reference and buffering rules.
...
For detailed recording examples and expressive cloning techniques, see [Voice Cloning Best Practices](/models/voice-cloning/best-practices).
...
- **Lexicon overrides.** Use [pronunciation dictionaries](/models/text-to-speech/pronunciation-dictionaries) for domain-specific terms, brand names, and acronyms where default grapheme-to-phoneme conversion is insufficient.

For comprehensive text formatting rules (numeric handling, date/time, symbols, chunking logic), see [TTS Best Practices](/models/text-to-speech/best-practices).
...
<Note>
  **Recommendations:** Use proper script for each language. Break long text at natural punctuation points. Use [pronunciation dictionaries](/models/text-to-speech/pronunciation-dictionaries) for specialized vocabulary. Test voice selection for your specific use case.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="Documentation" icon="book-open" href="/models/text-to-speech/quickstart" />
</CardGroup>
