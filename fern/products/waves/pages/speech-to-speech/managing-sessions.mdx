...
| `voice` | string | Voice ID. See the [Hydra model card](/models/model-cards/speech-to-speech/hydra) for the per-version roster. Omit the field and the server applies `sterling`. An unrecognised voice is rejected with an `error` frame (`code: "invalid_request_error"`, `param: "session.configure.session.voice"`), not silently defaulted. **Validate client-side.** |
