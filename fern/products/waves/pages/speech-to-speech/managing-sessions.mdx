...
| Field | Type | Notes |
|---|---|---|
| `instructions` | string | System prompt. See [Prompting voice agents](/models/speech-to-speech/prompting-voice-agents). |
| `voice` | string | Voice ID. Must match the model roster you connected with. On `?model=hydra-v1.1` (current release): one of `zoe`, `maya`, `elena`, `ivy`, `grace`, `alex`, `aria`, `leo`, `sam`, `kai`. On `?model=hydra` (original release): one of `wren`, `sloane`, `marlowe`, `reed`, `knox`, `tate`. Unknown values silently fall back to the default. **Validate client-side.** See the [Hydra model card](/models/model-cards/speech-to-speech/hydra) for the full per-version voice table. |
| `tools` | array | Function-calling tool schemas. See [Tool calling](/models/speech-to-speech/tool-calling). |
...
- [Audio I/O](/models/speech-to-speech/audio-i-o) - what to put in `input_audio_buffer.append` and how to play `response.output_audio.delta`
- [Turn detection & barge-in](/models/speech-to-speech/turn-detection-barge-in) - how speech events fire and how to handle interruption on the client
- [Tool calling](/models/speech-to-speech/tool-calling) - declare and execute functions during a session
