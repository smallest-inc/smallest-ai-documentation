...
Two Hydra versions are served on the same endpoint; pick one with `?model=`. The session protocol (event catalog, `session.configure` shape, tool calling, interruption handling) is identical between the two, so switching is a one-parameter change on the query string.

| Version | Query string | Notes |
|---|---|---|
| **`hydra-v1.1`** (current release) | `?model=hydra-v1.1` | Latest speech-to-speech model. Different voice roster from the original. |
| `hydra` (original release) | `?model=hydra` | Original release. Continues to serve requests that pass `model=hydra`; voice roster is unchanged. |

## Voices

Voice IDs are accepted on `session.configure.session.voice` and frozen at handshake. The roster depends on which model version you connect with.

### `hydra` (original release)
...
Ten voice IDs. The V1.1 roster is separate from the original `hydra` roster; when you switch a session to `?model=hydra-v1.1`, also pick a voice from this list.
...
|---|---|---|
| `wss://api.smallest.ai/waves/v1/s2s?model=hydra-v1.1` | WebSocket | Realtime full-duplex speech-to-speech (current release) |
| `wss://api.smallest.ai/waves/v1/s2s?model=hydra` | WebSocket | Realtime full-duplex speech-to-speech (original release) |
