...
<Tabs>
  ...
  <Tab title="Browser">
    ...
    ```
    The `WebSocket` global is supported in every modern browser. See [Audio I/O](/models/speech-to-speech/audio-i-o#browser-audioworklet) for the full mic + playback wiring with `AudioWorklet`.
  </Tab>
</Tabs>
...
The server now waits for one `session.configure` from you before accepting audio. Once configured, it echoes `session.configured` and starts accepting `input_audio_buffer.append` frames. See [Managing sessions](/models/speech-to-speech/managing-sessions).
...
- [Managing sessions](/models/speech-to-speech/managing-sessions) - `session.configure`, voices, persona, mid-session updates
- [Audio I/O](/models/speech-to-speech/audio-i-o) - input PCM16, output rate negotiation, browser AudioWorklet
- [Errors & reconnection](/models/speech-to-speech/errors-reconnection) - full error catalog
