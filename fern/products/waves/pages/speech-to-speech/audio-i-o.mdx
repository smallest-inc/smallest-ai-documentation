...
For barge-in, you reset `playCursor = playCtx.currentTime` when a fresh `response.created` arrives - see [Turn detection & barge-in](/models/speech-to-speech/turn-detection-barge-in#dropping-scheduled-audio-on-barge-in).
...
This pattern is for testing only - it doesn't exercise full-duplex behaviour (no overlap, no barge-in). For interactive use, see the [quickstart](/models/speech-to-speech/quickstart).

</Accordion>

## Next

- [Turn detection & barge-in](/models/speech-to-speech/turn-detection-barge-in) - VAD events and how to flush scheduled audio
- [Errors & reconnection](/models/speech-to-speech/errors-reconnection) - `invalid_audio` and friends
