...
<Note>
Three Hydra tags are served: `?model=hydra-v1.1` (current release, default), `?model=hydra-v1.0` (served, not deprecated), and `?model=hydra` (deprecated on 2026-08-20; sessions still open but the server emits a `warning` frame with `code: "model_deprecated"` on connect). Use `hydra-v1.1` on all new integrations. Full per-version voice reference: [Hydra model card](/models/model-cards/speech-to-speech/hydra). See also: [Deprecation Notices](/models/deprecations/notices).
</Note>
