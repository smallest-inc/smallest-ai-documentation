...
<Note>
Two Hydra versions are served on the same endpoint: pass `?model=hydra-v1.1` for the current release (10 voices: `zoe`, `maya`, `elena`, `ivy`, `grace`, `alex`, `aria`, `leo`, `sam`, `kai`), or `?model=hydra` for the original release (6 voices: `wren`, `sloane`, `marlowe`, `reed`, `knox`, `tate`). The session protocol is identical between the two, so switching is a one-parameter change on the query string; the voice roster is separate per version. Full per-version reference: [Hydra model card](/models/model-cards/speech-to-speech/hydra).
</Note>
