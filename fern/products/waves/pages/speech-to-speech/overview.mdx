...
<Note>
Use `?model=hydra-v1.1` on all new integrations. It's the current release and the default (10 voices: `zoe`, `maya`, `elena`, `ivy`, `grace`, `alex`, `aria`, `leo`, `sam`, `kai`). `?model=hydra` (the original release, 6 voices: `wren`, `sloane`, `marlowe`, `reed`, `knox`, `tate`) is **deprecated**: sessions still open, but the server emits a `warning` frame with `code: "model_deprecated"` on connect. Migrate to `?model=hydra-v1.1`. Full per-version reference: [Hydra model card](/models/model-cards/speech-to-speech/hydra). See also: [Deprecation Notices](/api-reference/deprecations/models).
</Note>
...
<CardGroup cols={2}>
  <Card title="Quickstart" icon="rocket" href="/models/speech-to-speech/quickstart">
    Clone the reference client, paste your API key, and talk to Hydra in your browser.
  </Card>
  <Card title="WebSocket connection" icon="plug" href="/models/speech-to-speech/web-socket-connection">
    Connect URL, auth, idle timeout, close codes. Python + Node + Browser snippets.
  </Card>
  <Card title="Managing sessions" icon="diagram-project" href="/models/speech-to-speech/managing-sessions">
    Session lifecycle, persona, voice, mid-session updates, conversation items.
  </Card>
  <Card title="Turn detection & barge-in" icon="wave-pulse" href="/models/speech-to-speech/turn-detection-barge-in">
    How the model detects speech, how to handle barge-in cleanly on the client.
  </Card>
  <Card title="Tool calling" icon="screwdriver-wrench" href="/models/speech-to-speech/tool-calling">
    Declare tools, stream arguments, post results back, narrate the answer.
  </Card>
  <Card title="Prompting voice agents" icon="masks-theater" href="/models/speech-to-speech/prompting-voice-agents">
