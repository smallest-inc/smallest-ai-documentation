...
|---|---|---|---|
| `?model=hydra` on `wss://api.smallest.ai/waves/v1/s2s` | 2026-08-20 | Deprecated. Session still opens, but the server emits a `warning` frame with `code: "model_deprecated"` immediately before `session.created`. The warning names `hydra-v1.0` as the migration target. | `?model=hydra-v1.1` (current release, recommended) or `?model=hydra-v1.0` (the tag the server's own warning points at). See the [Hydra model card](/models/model-cards/speech-to-speech/hydra) for the per-version voice roster. |
