...
| `stream` | bool | `false` | When `true`, response is Server-Sent Events. See [Streaming](/models/llm/streaming). |
...
| `tools` | array | - | Standard OpenAI function-calling tools array. Max 64 entries. See [Tool Calling](/models/llm/tool-function-calling). |
...
Unrecognized parameters not in the supported set are accepted by the schema but may be ignored by the model. See [Supported Parameters](/models/llm/supported-parameters) for the full passthrough table.
...
| `prompt_tokens_details.cached_tokens` | Subset of `prompt_tokens` served from prefix cache. Billed at the discounted rate. See [Prefix Caching](/models/llm/prefix-caching). |
...
When `stream: true`, the response is `text/event-stream` SSE. See [Streaming](/models/llm/streaming) for chunk format and example consumer code.
...
| 429 | Rate limit (RPM) or concurrency cap hit. See [Concurrency and Limits](/api-reference/concurrency-and-limits). |
