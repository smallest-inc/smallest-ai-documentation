...
| `choices[0].delta.tool_calls` | Partial function-call payload — see [Tool Calling: Streaming](/models/documentation/llm-electron/tool-function-calling#streaming). |
...
For voice-agent pipelines, start your TTS engine on the first `delta.content` chunk to mask end-to-end latency. See [Best Practices](/models/documentation/llm-electron/best-practices) and the [Voice Agent cookbook](/models/documentation/cookbooks/voice-agent-electron-pulse-lightning).
