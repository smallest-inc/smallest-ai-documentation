...
**Jump to:** [Benchmarks](#performance--benchmarks) · [Supported Languages](#supported-languages) · [API Reference](/api-reference/models/llm-chat-completions/complete) · [Pricing & Throughput](#throughput-latency--pricing) · [Quickstart](/models/llm/quickstart)
...
See [Electron - Chat Completions](/api-reference/models/llm-chat-completions/complete) for the full request/response schema, supported parameters, and error codes. The [Chat Completions guide](/models/llm/chat-completions) covers the OpenAI-compatible wire format end-to-end, and [Supported Parameters](/models/llm/supported-parameters) lists the passthrough table.
...
Both limits enforce strictly - over either cap returns `HTTP 429`. See [Concurrency & Limits](/api-reference/concurrency-and-limits) for full rate-limit semantics.

Pricing: Contact your Smallest AI account manager. Prefix-cache discounts apply automatically - see [Prefix Caching](/models/llm/prefix-caching). Every response reports `usage.prompt_tokens_details.cached_tokens` so you can audit cache hit rates.
...
- **Stream when you can.** Set `"stream": true` and start your TTS engine on the first `delta.content` chunk to mask end-to-end latency. See [Streaming](/models/llm/streaming).
- **Put repeated context at the prompt prefix.** System prompts, RAG context, and conversation history live in the cached prefix automatically. See [Prefix Caching](/models/llm/prefix-caching).
- **For voice agents, prompt for a filler phrase before tool calls.** Electron emits the filler in `content` alongside `tool_calls`, so your TTS can speak it while the tool runs. See [Tool Calling](/models/llm/tool-function-calling).
