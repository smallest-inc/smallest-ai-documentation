...
See [Prefix Caching](/models/documentation/llm-electron/prefix-caching) for the full guide.
...
The user hears *"Let me check the weather for you…"* while your weather API resolves in the background. Perceived latency drops by hundreds of milliseconds. See [Tool Calling: voice-agent pattern](/models/documentation/llm-electron/tool-function-calling#voice-agent-pattern-the-reason-this-exists).
...
- **Voice transcription** → [Pulse](/models/documentation/speech-to-text-pulse/overview)
- **Speech synthesis** → [Lightning](/models/documentation/text-to-speech-lightning/overview)
- **Full voice-agent platform with built-in workflow tooling** → [Atoms](/voice-agents/platform/get-started/quick-start). Electron is the LLM behind many Atoms agents; Atoms is the right choice when you want the platform-level scaffolding (campaigns, knowledge base, telephony) rather than building it yourself.
