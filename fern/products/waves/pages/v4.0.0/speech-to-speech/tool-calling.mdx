...
```
client → session.configure                       (tools: [...])
...
server ← response.function_call_arguments.done   { call_id, name, arguments }

client → conversation.item.create                (function_call_output, call_id, output)
client → response.create                         (request narration)

server ← response.created                        (narration response)
server ← response.output_audio.delta             (the bot narrates the result)
...
- **Tools execute on your side, not Hydra's.** Hydra streams arguments; you run the code. Same model as the OpenAI Realtime API and Gemini Live API.
