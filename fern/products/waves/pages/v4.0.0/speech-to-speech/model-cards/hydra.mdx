...
Hydra is benchmarked head-to-head against a set of production-grade realtime voice / speech-to-speech models.

### AIEWF S2S — 10 runs × 30 turns, `aiwf_medium_context`

| Metric | Hydra |
|---|---|
| **Pass rate** | **95.9 %** *(top 3 of 9 in the comparison set, within ~2 pp of the leader)* |
| **Non-tool voice-to-voice median latency** | **864 ms** *(tied-fastest in the comparison set)* |
| **Non-tool voice-to-voice max latency** | 1984 ms |
| **Tool voice-to-voice mean latency** | **1624 ms** *(fastest in the comparison set)* |

Latency numbers come from `transcript.jsonl` across all 10 runs (n = 224 non-tool turns, n = 64 tool turns). Pass rate is the fraction of turns that completed the expected interaction.
