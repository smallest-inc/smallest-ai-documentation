...
<Info>
The voice catalog is in preview and voice IDs may be renamed before general availability. Watch the [changelog](/waves/changelog) for a migration notice ahead of any rename.
</Info>

## Performance & benchmarks

Hydra is benchmarked against eight other production-grade voice / realtime models — Ultravox v0.7, OpenAI gpt-realtime (1.0 / 1.5 / 2-low), Gemini Live, Gemini 3.1 Flash Live Preview, Grok Voice Think Fast 1.0, and Amazon Nova-2-Sonic.
...
Hydra is **#3 of 9** on pass rate (within ~2 pp of the leader), **tied-fastest** on non-tool voice-to-voice median, and **fastest of 9** on tool voice-to-voice mean. Latency numbers are from `transcript.jsonl` across all 10 runs (n = 224 non-tool turns, n = 64 tool turns).
