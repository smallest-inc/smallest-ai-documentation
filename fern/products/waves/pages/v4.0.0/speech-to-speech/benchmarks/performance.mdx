...
- **Single benchmark.** The AIEWF S2S benchmark is one measurement under a fixed methodology. Real-world workloads can shift relative standings.
- **Pass rate is task-dependent.** A 95.9 % pass rate here doesn't predict 95.9 % on every prompt distribution — the benchmark uses `aiwf_medium_context`, which is a balanced mix of short and multi-turn prompts.
- **Latencies are server-side.** They don't include client-side audio capture, network egress to the WebSocket, or speaker playback latency. End-to-end glass-to-glass latency on a real device is typically 100–300 ms higher.

## Next

<CardGroup cols={2}>
  <Card title="Metrics Overview" icon="ruler" href="/waves/documentation/speech-to-speech-hydra/benchmarks/metrics-overview">
