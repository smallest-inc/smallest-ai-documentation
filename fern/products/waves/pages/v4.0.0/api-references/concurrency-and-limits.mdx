...
**Concurrency** refers to the number of simultaneous requests that can be processed at any given moment. Concurrency caps are **plan-based** and apply per-account across all Waves models (Pulse, Pulse Pro, Lightning v3.1, Lightning v3.1 Pro, Hydra, Electron).

- **Standard plan**: 1 concurrent TTS request, 25 RPM per STT model. WebSocket pool of up to 5 connections (only 1 active request across them at a time).
- **Enterprise plan**: configurable per-customer; STT is unlimited and TTS scales to the customer's dedicated capacity. The model-card benchmarks (e.g. ~200 ms TTFB at 40 concurrent TTS, ~300 ms STT TTFT at 100 concurrent) are run on Enterprise-tier capacity.

Contact your account manager for Enterprise pricing and capacity sizing.
...
- If you attempt a second HTTP request while one is already being processed (on the Standard plan), you'll receive a `429 Too Many Requests` error

### WebSocket Connections

- The Standard plan allows up to **5 WebSocket connections** simultaneously (5 × concurrency limit)
- Only **1 concurrent request** can be processed across all WebSocket connections on Standard
- Additional requests sent through any WebSocket while one is being processed will be rejected with an error
- Enterprise-tier limits scale these caps per-customer; see your account manager
