...
If your code already passes `language=multi-indic` or `language=multi-asian` to the **pre-recorded HTTP endpoint**, no action needed. That path is correct. If you wired `multi-indic` into a **WebSocket streaming** call, switch to `north_indic` (the streaming-side Indic auto-detect) or to specific single-language codes (`en`, `hi`, `gu`, `mr`, `bn`, `or`). `multi-asian` is supported on streaming in the US region only.

→ [Pulse STT pre-recorded](/models/documentation/speech-to-text-pulse/pre-recorded/quickstart)
