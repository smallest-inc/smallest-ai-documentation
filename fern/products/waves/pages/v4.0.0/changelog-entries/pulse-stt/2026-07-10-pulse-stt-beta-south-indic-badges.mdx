...
Tamil, Telugu, Kannada, and Malayalam (plus the `multi-south-indic` aggregator) are now marked **Beta** on the [Pulse model card](/waves/model-cards/speech-to-text/pulse) and the [STT overview](/waves/documentation/speech-to-text-pulse/overview). Accuracy improvements are ongoing. These codes remain India-region only (`wss://api.smallest.ai/waves/v1/stt/live`); the US endpoint continues to reject them with `LANGUAGE_NOT_ENABLED_IN_REGION`.
...
→ [Pulse model card](/waves/model-cards/speech-to-text/pulse#supported-languages)
