...
- Each language has its own dedicated Pro voices — see the [Lightning v3.1 Pro voice catalog](/waves/model-cards/text-to-speech/lightning-v-3-1-pro#voice-catalog) for the full per-language list with voice IDs and genders.
- `language: en`, `language: hi`, and the omit-`language` default (`en + hi`) behave exactly as before.
- The full updated voice list is available via the [Get Voices endpoint](/waves/api-reference/api-reference/text-to-speech/get-waves-voices).
