...
date: 2026-06-01
description: Opt in with `word_timestamps: true` on a WebSocket request to receive interleaved `status: "word_timestamp"` frames with per-word `{id, word, start, end}` timing — verbatim from input text, supported on Lightning v3.1 and v3.1 Pro base-queue English + Hindi voices.
---
