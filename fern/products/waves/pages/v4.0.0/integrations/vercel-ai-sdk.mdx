...
const result = await transcribe({
  ...
  providerOptions: {
    smallestai: {
      ...
      itnNormalize: true,        // recommended over `numerals` — converts spoken numbers, dates, currencies, phone numbers, and other entities into written form (Pulse STT WS only)
    },
  },
});
```
...
<Note>
`itnNormalize` requires the underlying [Smallest provider for Vercel AI SDK](https://github.com/smallest-inc/smallest-ai-vercel) to forward the option to the Pulse STT WebSocket layer. If you're on an older provider release that doesn't whitelist this key, the option is silently dropped client-side. Verify against the provider's release notes if `itnNormalize` doesn't take effect.
</Note>
