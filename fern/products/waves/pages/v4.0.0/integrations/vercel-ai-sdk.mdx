...
Latest: **`smallestai-vercel-provider@0.6.0`** — adds browser-native streaming (no proxy required), microphone capture hooks, auto-reconnect on socket drops, and a security-validated `signedUrl` flow for production browser apps.
