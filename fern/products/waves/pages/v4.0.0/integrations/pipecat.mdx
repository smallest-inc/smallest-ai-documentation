...
| `word_timestamps` | `bool` | `None` | Opt in to per-word timing events on WebSocket streaming. Supported on base-queue English + Hindi voices (`meher`, `devansh`, `kartik`, `maithili`, `liam`, `avery`). Other voices silently emit no word events. |
