...
The STT service connects to the Pulse real-time WebSocket endpoint (`wss://api.smallest.ai/waves/v1/stt/live?model=pulse`) and streams audio frames from the pipeline, returning transcriptions with ~150 ms streaming TTFT at 1 concurrency.
...
| `voice`    | `str`              | model-dependent      | Voice ID. Plugin defaults: `meher` for `lightning_v3.1_pro`, `magnus` for `lightning_v3.1` |
