...
- **`smallestai.STT`** — real-time speech-to-text using the Pulse API, with streaming over WebSocket (~150 ms TTFT at 1 concurrency) and batch transcription over HTTP
...
# Automatic language detection across Indic languages
stt = smallestai.STT(language="multi-indic")
...
| `language` | `str` | `"en"` | BCP-47 language code (e.g. `"en"`, `"hi"`, `"fr"`). Use `"multi-eu"` (European set) or `"multi-indic"` (Indic set) for regional auto-detection |
...
| `voice_id` | `str` | auto | Voice ID for synthesis. Defaults to `"meher"` for `lightning_v3.1_pro` and `"magnus"` for `lightning_v3.1`. Pro voices must be paired with `lightning_v3.1_pro`; standard voices with `lightning_v3.1` — see the [Lightning v3.1 voice catalog](/waves/model-cards/text-to-speech/lightning-v-3-1#voice-catalog) and [Lightning v3.1 Pro voice catalog](/waves/model-cards/text-to-speech/lightning-v-3-1-pro#voice-catalog) |
| `language` | `str` | `"en"` | Language code. `lightning_v3.1` supports 12 codes; `lightning_v3.1_pro` supports `"en"` (UK + American accented English) or `"hi"` (Indian English + Hindi code-switching), or omit to default to `en + hi` auto-detect — see the [model cards](/waves/model-cards/text-to-speech) for the full lists |
