...
The full runnable example is in the Smallest AI waves-examples repository:

[LiveKit Example — Smallest AI TTS + STT](https://github.com/smallest-inc/waves-examples/tree/main/lightning/livekit_example)
...
## Testing TTS in Isolation

To verify your `SMALLEST_API_KEY` and TTS setup without running a full agent, use `generate_audio.py` from the waves-examples repository to synthesize a sentence and save it as a WAV file:

```bash
git clone https://github.com/smallest-inc/waves-examples.git
cd waves-examples/lightning/livekit_example
pip install -r requirements.txt
python3 generate_audio.py
```

Edit `voice_id`, `text`, or other parameters in the script to try different voices and languages. The output is saved as `output.wav`.
...
- For issues or questions, open an issue in the [waves-examples repository](https://github.com/smallest-inc/waves-examples) or reach out on [Discord](https://discord.gg/9WtSXv26WE).
