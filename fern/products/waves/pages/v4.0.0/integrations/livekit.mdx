...
| `language` | `str` | `"en"` | Language code (e.g. `"en"`, `"hi"`, `"fr"`). LiveKit uses Pulse's streaming surface. Use `"north_indic"` (Indian-language auto-detect) or `"multi-asian"` (East Asian auto-detect, US region only) when the language is unknown. See the [Pulse model card](/models/model-cards/speech-to-text/pulse) for the full streaming code list |
