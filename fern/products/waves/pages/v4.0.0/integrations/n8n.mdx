...
<Steps>
  <Step title="Sign up and navigate to API Keys">
    Sign up at [app.smallest.ai](https://app.smallest.ai/dashboard) and open the [API Keys page](https://app.smallest.ai/dashboard/api-keys?utm_source=documentation&utm_medium=n8n-integration) directly.
...
| --- | --- | --- |
| `language` | `en` | Set to a regional aggregator (`north_indic`, `multi-eu`, `multi-asian`) for automatic detection |
| `diarize` | `false` | Speaker diarization on pre-recorded transcription |
| `word_timestamps` | `false` | Per-word start/end timestamps |
...
**Supported languages:** English, Hindi, Spanish, French, German, Bengali, Marathi, Gujarati, and more (17 streaming + 26 pre-recorded languages). See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for the full list. Set `language` to a regional aggregator (`north_indic`, `multi-eu`, `multi-asian`) for automatic detection within a language family.
...
| Speaker diarization | `diarize` | `false` |
