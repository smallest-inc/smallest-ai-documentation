...
| `language` | string | `en` | Language code. `auto` for code-switching detection. Indian: `en`, `hi`, `mr`, `kn`, `ta`, `bn`, `gu`, `te`, `ml`, `pa`, `or`. European: `de`, `fr`, `es`, `it`, `pl`, `nl`, `ru`, `sv`, `pt`. MENA: `ar`, `he`. See the [Lightning v3.1 model card](/waves/model-cards/text-to-speech/lightning-v-3-1) for the full list. |
