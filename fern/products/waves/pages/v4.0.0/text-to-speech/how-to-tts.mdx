...
| `language` | string | `en` | Language code. `auto` for code-switching detection. Indian: `en`, `hi`, `mr`, `kn`, `ta`, `bn`, `gu`, `te`, `ml`, `pa`, `or`. European: `es`. Each voice supports a subset — see voice tags via `GET /waves/v1/lightning-v3.1/get_voices`. |
| `output_format` | string | `pcm` | Audio format: `pcm`, `wav`, `mp3`, `ulaw`, or `alaw` |
| `pronunciation_dicts` | array | — | List of [pronunciation dictionary](/waves/documentation/text-to-speech-lightning/features/pronunciation-dictionaries) IDs |
