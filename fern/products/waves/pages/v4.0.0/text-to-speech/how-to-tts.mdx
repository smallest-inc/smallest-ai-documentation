...
| `voice_id` | string | *required* | Voice to use. Examples: `magnus`, `olivia`, `aarush` (standard Lightning v3.1 catalog); `meher`, `kaitlyn`, `henry` (Lightning v3.1 Pro catalog). Pair Pro voices with `"model": "lightning_v3.1_pro"`. See the [voice catalog](/waves/documentation/text-to-speech-lightning/voices) for the full list. |
| `model` | string | `lightning_v3.1` | TTS pool to use. Pass `lightning_v3.1_pro` to route to the [Pro pool](/waves/model-cards/text-to-speech/lightning-v-3-1-pro). |
