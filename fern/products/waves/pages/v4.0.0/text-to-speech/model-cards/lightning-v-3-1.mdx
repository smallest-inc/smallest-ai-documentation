...
**Legacy (deprecated — no longer documented, but still callable for existing integrations):**

The model-name-based routes below still function on the platform for backwards compatibility with older SDKs and integrations. They are no longer documented in the API reference — new work should use the unified routes above with `model: lightning_v3.1`.

| Endpoint | Method | Migration target |
|----------|--------|------------------|
| `https://api.smallest.ai/waves/v1/lightning-v3.1/get_speech` | POST | `POST /waves/v1/tts` with `model: lightning_v3.1` |
| `https://api.smallest.ai/waves/v1/lightning-v3.1/stream` | POST (SSE) | `POST /waves/v1/tts/live` with `model: lightning_v3.1` |
| `wss://api.smallest.ai/waves/v1/lightning-v3.1/get_speech/stream` | WebSocket | `wss://api.smallest.ai/waves/v1/tts/live` with `model: lightning_v3.1` |
...
| `WSS /waves/v1/lightning-v3.1/get_speech/stream` (legacy, no longer documented) | ✅ |
