...
**Jump to:** [Benchmarks](#performance--benchmarks) · [Voice Catalog](#voice-catalog) · [API Reference](/waves/api-reference/api-reference/text-to-speech/synthesize-speech) · [Pricing & Throughput](#throughput-latency--pricing) · [Quickstart](/waves/documentation/text-to-speech-lightning/quickstart)
...
<CardGroup cols={4}>
  ...
  <Card title="29 Languages" icon="globe" href="#supported-languages">
    English + Hindi, plus 27 more — 9 Indian, 8 Asian & Middle Eastern, 10 European
  </Card>
...
| **Languages** | English (`en` → UK + American accent), Hindi (`hi` → Indian English + Hindi code-switching). Omit `language` → defaults to `en + hi`. Plus 27 additional languages — 9 Indian, 8 Asian & Middle Eastern, 10 European — via dedicated Pro voices; see [Supported Languages](#supported-languages). |
| **License** | Proprietary |
| **Version** | v3.1 Pro |
...
<Capabilities bordered>
  ...
  <Capability icon="globe" name="Multi-Language" href="#supported-languages">
    `language: en` → UK + American accented English. `language: hi` → Indian accented English + Hindi (code-switching). Omit → defaults to `en + hi`. Plus 27 additional languages via dedicated Pro voices — pass the ISO 639-1 code (e.g. `ta`, `de`, `ja`).
  </Capability>
...
27 additional languages have dedicated Pro voices. Pass the ISO 639-1 code in the `language` body parameter and pick a `voice_id` from the matching [Voice Catalog](#voice-catalog) section.

| Group | Language | Code | Pro voices |
|---|---|---|---:|
| Indian | Marathi | `mr` | 6 |
| Indian | Tamil | `ta` | 12 |
| Indian | Malayalam | `ml` | 6 |
| Indian | Telugu | `te` | 8 |
| Indian | Kannada | `kn` | 10 |
| Indian | Punjabi | `pa` | 7 |
| Indian | Bengali | `bn` | 5 |
| Indian | Odia | `or` | 8 |
| Indian | Gujarati | `gu` | 5 |
| Asian & Middle Eastern | Arabic | `ar` | 2 |
| Asian & Middle Eastern | Chinese (Mandarin) | `zh` | 5 |
| Asian & Middle Eastern | Indonesian | `id` | 4 |
| Asian & Middle Eastern | Japanese | `ja` | 4 |
| Asian & Middle Eastern | Korean | `ko` | 1 |
| Asian & Middle Eastern | Malay | `ms` | 2 |
| Asian & Middle Eastern | Turkish | `tr` | 2 |
| Asian & Middle Eastern | Vietnamese | `vi` | 1 |
| European | German | `de` | 7 |
| European | Spanish | `es` | 6 |
| European | French | `fr` | 9 |
| European | Italian | `it` | 6 |
| European | Portuguese (Brazilian + European) | `pt` | 7 |
| European | Russian | `ru` | 7 |
| European | Greek | `el` | 5 |
| European | Finnish | `fi` | 6 |
| European | Norwegian | `no` | 4 |
| European | Polish | `pl` | 4 |
...
| `language` | string | No | *omit → `en + hi`* | `en` → UK + American accented English; `hi` → Indian accented English + Hindi (code-switching); omit → `en + hi`. Also accepts the ISO 639-1 code of any of the 27 additional Pro languages (e.g. `ta`, `de`, `ja`) when paired with a voice from that language — see [Supported Languages](#supported-languages). |
...
| `word_timestamps` | boolean | No | `false` | Opt in to per-word timing events on WebSocket. See [Word-level timestamps on the standard v3.1 model card](/waves/model-cards/text-to-speech/lightning-v-3-1#word-level-timestamps) — same wire shape, same opt-in flag. Pro shares the Lightning v3.1 base-queue voices for English + Hindi (`meher`, `devansh`, `kartik`, `maithili`, `liam`, `avery`), so word events emit on those; other Pro-only voices fall back to silent graceful degradation (audio normal, no word events). |
...
Pro runs on dedicated inference capacity isolated from general Lightning traffic. Concurrency, latency, and rate-limit ceilings are identical to standard [Lightning v3.1](/waves/model-cards/text-to-speech/lightning-v-3-1); the difference is voice quality and catalog. Rate limits, concurrency caps, and pricing tiers are documented on the [Concurrency & Limits](/waves/api-reference/api-references/concurrency-and-limits) page. For enterprise pricing, contact [sales@smallest.ai](mailto:sales@smallest.ai).
