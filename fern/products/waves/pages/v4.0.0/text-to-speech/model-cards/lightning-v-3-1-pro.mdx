...
| **Languages** | English (`en` → UK + American accent), Hindi (`hi` → Indian English + Hindi code-switching). Omit `language` → defaults to `en + hi`. |
| **License** | Proprietary |
| **Version** | v3.1 Pro |
...
| `language` | string | No | *omit → `en + hi`* | `en` → UK + American accented English; `hi` → Indian accented English + Hindi (code-switching); omit → `en + hi`. |
