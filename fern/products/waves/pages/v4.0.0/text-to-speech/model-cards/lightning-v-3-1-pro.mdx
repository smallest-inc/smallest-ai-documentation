...
|---|---|---|
| Indian Pro voices | English (`en`), Hindi (`hi`) | English ↔ Hindi mid-utterance handled natively when the voice's `tags.language` covers both |
...
| `language` | string | No | `"en"` | `en` or `hi`, matching the voice's `tags.language`. |
