...
```python Python SDK
# ci:skip — WavesStreamingTTS hardcodes the deprecated lightning-v2 WS URL
# and a non-canonical host. Fix ships in the next SDK release (see PR #128
# coordination). Use the cURL / Node.js examples above as the canonical
# pattern until then.
import os
import wave
...
```python
# ci:skip — same SDK URL/host issue as the Python SDK block above; ships
# in next SDK release. The pattern below is correct — the WS connect will
# work once the SDK is updated.
import os
from smallestai.waves import TTSConfig, WavesStreamingTTS
