...
description: "Stream TTS audio in real-time via WebSocket or SSE — ~200 ms TTFB at 40 concurrent on the production benchmark."
icon: "bars-staggered"
---

Streaming TTS delivers audio chunks as they're generated — playback starts immediately instead of waiting for the full file. ~200 ms time-to-first-byte (TTFB) at 40 concurrent requests on the production benchmark; see the [Lightning v3.1 model card](/waves/model-cards/text-to-speech/lightning-v-3-1) for the full table.
...
# The current SDK release targets /waves/v1/lightning-v3.1/get_speech/stream
# (standard pool). Use the raw `websockets` example above to access the
# Lightning v3.1 Pro pool until the SDK ships `model` field support.
import os
import wave
...
| `model` | `lightning_v3.1` | TTS pool to use. Pass `lightning_v3.1_pro` to route to the [Pro pool](/waves/model-cards/text-to-speech/lightning-v-3-1-pro). |
