...
<Note>
This quickstart uses **Pulse Pro**, our leaderboard-ranked English model. If you need multilingual transcription (15 languages including English, Hindi, German, Spanish, Russian, Italian, French, Dutch, Portuguese, Mandarin, Cantonese, Japanese, Korean, Gujarati, Marathi) or streaming, swap `?model=pulse-pro` for `?model=pulse`. See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for the full list and the `multi-eu` / `multi-indic` aggregators.
</Note>
...
<Note>
For Pulse Pro, `language` must be `en`. For Pulse, set `language` to the known code (`en`, `hi`, `es`, etc.) for best accuracy, or use an aggregator (`multi-eu`, `multi-indic`) for mixed-language audio.
</Note>
