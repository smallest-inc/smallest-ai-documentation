...
import { VadEventsDemo } from "@/components/vad-events-demo/VadEventsDemo";
...
Press play on each sample to hear the audio and watch the captured server messages stream in sync. The three event streams were captured from live production runs and embedded directly in this page.

<VadEventsDemo />

The third sample illustrates the trailing-silence behavior described above: `speech_ended` only fires after the model observes silence.
