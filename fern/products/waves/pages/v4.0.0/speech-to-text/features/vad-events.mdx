...
Press play on each sample to hear the audio and watch the captured server messages stream in sync. All three event streams below were captured from live production runs.

<VadEventsDemo />

The third sample shows the behavior documented in [Notes](#notes): `speech_ended` only fires after the model observes silence.
