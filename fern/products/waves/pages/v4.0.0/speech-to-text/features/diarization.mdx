...
Pass `diarize=true` when calling the Pulse STT POST endpoint. The parameter can be combined with other request params (`word_timestamps=true`, `redact_pii=true`, `redact_pci=true`) without changing your audio payload. Sentence-level segments (the `utterances` array on the response) come back automatically on `?model=pulse` whenever `word_timestamps=true` is set — there is no separate `utterances` request param.
