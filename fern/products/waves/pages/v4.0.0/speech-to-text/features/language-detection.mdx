...
description: 'Automatically detect and transcribe across the European, Indic, or East Asian Pulse STT language sets'
---
...
Pick the aggregator that matches the audio you expect; on pre-recorded `multi-eu` is the safer default for unknown European audio, and on streaming `north_indic` is the safer default for Indian-language audio. When you already know the source language, pass the explicit code (`en`, `hi`, `es`, etc.) for the best accuracy.

<Note>
For the exact codes inside each aggregator + region notes, see the [Pulse model card](/waves/model-cards/speech-to-text/pulse#supported-languages---non-streaming) — the single source of truth for which languages are supported on streaming vs batch.
</Note>
