...
When present, the `from_finalize` boolean on a final transcript indicates whether that transcript was triggered by a manual `{"type":"finalize"}` message (`true`) or by automatic finalization (`false`). Use it to attribute downstream logic to the correct trigger.
