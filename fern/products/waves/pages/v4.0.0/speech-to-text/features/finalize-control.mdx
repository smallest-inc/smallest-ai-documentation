...
// Pulse emits an is_final transcript for the pending chunk.
...
- [Inverse Text Normalization (ITN)](/models/documentation/speech-to-text-pulse/features/inverse-text-normalization) — `finalize_on_words` is most useful in combination with ITN.
- [End-of-Utterance Timeout](/models/documentation/speech-to-text-pulse/features/end-of-utterance-timeout) — controls automatic finalization timing when `finalize_on_words` stays `true`.
- [Pulse STT WebSocket reference](/models/api-reference/api-reference/speech-to-text/speech-to-text) — full parameter reference and the `finalize` / `close_stream` control messages.
