...
<Note>
ITN normalizes **finalized transcripts** (`is_final: true`). For the best results in voice agent and conversational AI pipelines:

1. **Disable word-count finalization**: Set `finalize_on_words=false` so the server doesn't split utterances mid-phrase
2. **Match EOU timeout to your VAD**: Set `eou_timeout_ms` to match your Voice Activity Detection silence threshold (e.g., 600ms). The EOU timeout controls when the pipeline finalizes after detecting a pause — ITN normalizes these EOU-triggered finals
3. **Use manual finalize to end the stream**: Send `{"type": "finalize"}` to flush remaining audio when the conversation turn ends

This gives you natural utterance boundaries based on speech pauses rather than arbitrary word counts, producing cleaner ITN results for downstream LLM input, form filling, and command parsing.
</Note>

<Warning>
**Manual finalize limitation**: Currently, transcripts triggered by `{"type": "finalize"}` messages may not be ITN-normalized. ITN runs reliably on EOU-timeout-triggered finals and word-count-triggered finals. If you need ITN on the final chunk, ensure there's a natural pause before sending the finalize message so the EOU timeout triggers first.
</Warning>
...
Disable word-count finalization and match `eou_timeout_ms` to your VAD threshold. The EOU timeout triggers finalization on natural speech pauses, and ITN normalizes each finalized chunk:
...
params = {
    ...
    "sample_rate": "16000",
    "itn_normalize": "true",
    "finalize_on_words": "false",   # Disable word-count finalization
...
async def transcribe_agentic(audio_file: str):
    ...
    async with websockets.connect(WS_URL, additional_headers=headers) as ws:
        ...
        # End the stream — wait briefly so EOU can finalize the last chunk
        ...
        async for message in ws:
            ...
            if data.get("is_final"):
                print(f"Final: {data['transcript']}")
                # EOU-triggered: "$25", "January 15 2026"
            if data.get("is_last"):
                break
```
