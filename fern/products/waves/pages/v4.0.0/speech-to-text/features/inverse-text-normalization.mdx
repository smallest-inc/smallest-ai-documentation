...
3. When your VAD detects that the user has stopped speaking, send `{"type": "close_stream"}` — this closes the stream, ITN normalizes the entire accumulated chunk, and emits a final `is_last=true` transcript
...
Disable internal word-count finalization, set `eou_timeout_ms` to match your VAD, and send `{"type": "close_stream"}` when your VAD detects end-of-speech. This closes the stream, flushes the final chunk, ITN normalizes it as a whole, and emits `is_last=true` so the receive loop can exit:
...
async def transcribe_agentic(audio_file: str):
    ...
    async with websockets.connect(WS_URL, additional_headers=headers) as ws:
        ...
        # VAD detected end-of-speech → close_stream
        # (close_stream is the only message that emits is_last=true; the
        # protocol's `finalize` only forces an interim is_final=true)
        await ws.send(json.dumps({"type": "close_stream"}))
