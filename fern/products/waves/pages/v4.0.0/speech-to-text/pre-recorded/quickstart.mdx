...
For non-English audio, code-switching, or when you need streaming, use `?model=pulse`. Set `language` explicitly to the known code (`en`, `hi`, `es`, etc.) for best accuracy, or use a regional aggregator for unknown audio: on pre-recorded that's `multi-eu` (21 European codes + `en`) or `multi-asian` (`zh`, `ja`, `ko`, `en`). See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for the full per-mode language matrix.
