...
The legacy path `POST /waves/v1/pulse/get_text` still works but is deprecated; migrate to `/waves/v1/stt/`.
...
For long audio files where you do not want to hold an HTTP connection open, pass `webhook_url`. The endpoint returns `202` immediately, and the transcription hits your webhook when ready.
