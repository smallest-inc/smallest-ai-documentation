...
<Note>
For Pulse, set `language` explicitly to match the audio (`en`, `hi`, `es`, etc.) for the best accuracy. For unknown audio, pick the regional auto-detect scope: `multi-eu` (de, en, fr, it, nl, pt, ru, es) or `multi-indic` (en, hi, mr, pa, gu, or, ka, ta, te, ml, bn).
</Note>
