...
<CardGroup cols={2}>
  <Card title="Pulse model card" icon="microphone" href="/waves/model-cards/speech-to-text/pulse">
    Multilingual streaming + non-streaming. 31 documented languages (17 streaming + 26 pre-recorded) + regional aggregators.
  </Card>
...
<AccordionGroup>
  <Accordion title="31 Languages & Automatic Language Detection">
    31 documented languages across streaming + pre-recorded modes (17 on streaming, 26 on pre-recorded). Set `language` to a known code (`en`, `hi`, etc.) for best accuracy, or use a regional aggregator for unknown audio: `multi-eu` on batch, `north_indic` on streaming, and `multi-asian` (streaming + US region only on the WebSocket surface). See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for the full list with regional notes.
  </Accordion>
  ...
  <Accordion title="Low Latency">
    Streaming pipeline delivering ~150 ms streaming TTFT at 1 concurrency (~300 ms at 100). Tuned for real-time conversational AI and live captioning. See [Measuring latency](/waves/documentation/speech-to-text-pulse/measuring-latency) for the methodology.
  </Accordion>
</AccordionGroup>
...
**Streaming (Real-Time, WebSocket) — 17 single-language codes + 2 regional aggregators:**

`en`, `hi`, `de`, `es`, `ru`, `it`, `fr`, `nl`, `pt`, `zh`, `yue`, `ja`, `ko`, `gu`, `mr`, `or`, `bn`, plus `north_indic` (auto-detects across en/hi/gu/mr/bn/or) and `multi-asian` (auto-detects across zh/yue/ko/ja/en; US region only — contact sales for access in the India region).
