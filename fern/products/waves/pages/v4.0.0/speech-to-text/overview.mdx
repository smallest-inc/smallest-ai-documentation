...
| Lithuanian | `lt` |
| Maltese | `mt` |
| Japanese | `ja` &mdash; accepted but unreliable \[1] |
| Mandarin | `zh` &mdash; accepted but unreliable \[1] |
| Korean | `ko` &mdash; accepted but unreliable \[1] |
| Tagalog | `tl` |
| Indonesian | `id` |
| Malay | `ms` |

\[1] `ja`, `zh`, `ko`, and the `multi-asian` aggregator are accepted by batch (HTTP 200) but transcripts are unreliable — the batch model lacks proper checkpoints for East Asian audio and tends to return garbled CJK-mixed output. Cantonese (`yue`) is the one exception: batch returns `400 LANGUAGE_NOT_ENABLED_IN_REGION` and is not in the enum. For accurate Asian-language transcription, use the streaming endpoint on `wss://api.us.smallest.ai/waves/v1/stt/live`.
