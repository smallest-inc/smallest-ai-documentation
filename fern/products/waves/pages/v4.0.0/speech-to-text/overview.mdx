...
`en`, `hi`, `de`, `es`, `ru`, `it`, `fr`, `nl`, `pt`, `zh`, `yue`, `ja`, `ko`, `gu`, `mr`, `or`, `bn`, plus `north_indic` (auto-detects across en/hi/gu/mr/bn/or) and `multi-asian` (auto-detects across zh/yue/ko/ja/en; US region only — contact sales).
