...
- **`?model=pulse`**: multilingual (15 languages), supports streaming and pre-recorded transcription.
...
<CardGroup cols={2}>
  <Card title="Pulse model card" icon="microphone" href="/waves/model-cards/speech-to-text/pulse">
    Multilingual streaming + non-streaming. 15 languages plus `multi-eu` / `multi-indic` aggregators.
  </Card>
...
<AccordionGroup>
  <Accordion title="15 Languages & Multi-language Aggregators">
    Support for 15 languages — English, Hindi (with English code-switching), German, Spanish, Russian, Italian, French, Dutch, Portuguese, Mandarin, Cantonese, Japanese, Korean, Gujarati, Marathi. Pass ISO 639-1 codes (`en`, `hi`, `de`, etc.) or use `language=multi-eu` (en + de + es + it + pt + nl + ru + fr) or `language=multi-indic` (en + hi + gu + mr) for mixed-language audio.
  </Accordion>
...
Pulse supports the same 15 individual languages in both pre-recorded (HTTP) and real-time (WebSocket) modes, plus two multi-language aggregators.
...
### Multi-language aggregators

For mixed-language audio, pass an aggregator instead of an individual code. Pulse routes to the right model internally.

| Code | Covers |
|---|---|
| `multi-eu` | en + de + es + it + pt + nl + ru + fr |
| `multi-indic` | en + hi + gu + mr |
