...
| **Languages** | 39 supported on streaming (35 on batch); `multi-eu`, `multi-indic`, and `multi-asian` aggregators for unknown audio (`multi-asian` is US-region only on streaming) |
| **License** | Proprietary |
...
| Lithuanian | `lt` | Yes |
| Maltese | `mt` | Yes |
| Japanese | `ja` | Yes &mdash; US region only \[1] |
| Cantonese | `yue` | Yes &mdash; US region only \[1] |
| Mandarin | `zh` | Yes &mdash; US region only \[1] |
| Korean | `ko` | Yes &mdash; US region only \[1] |
| Tagalog | `tl` | Yes |
| Indonesian | `id` | Yes |
| Malay | `ms` | Yes |

\[1] These four languages are served from the US region only. Connect to `wss://api.us.smallest.ai/waves/v1/pulse/get_text` (or `/waves/v1/stt/live`) instead of the default `wss://api.smallest.ai/...` host. Requesting them on the default (ap-south-1) host closes the connection without a transcription frame.
