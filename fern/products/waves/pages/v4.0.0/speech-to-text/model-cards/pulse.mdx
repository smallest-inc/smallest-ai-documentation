...
| **Languages** | 15 documented languages across both modes (Cantonese is streaming-only); `multi-eu`, `multi-indic`, and `multi-asian` aggregators for unknown audio (`multi-asian` is streaming + US region only) |
| **License** | Proprietary |
...
<CardGroup cols={3}>
  ...
  <Card title="Multi-Language" icon="globe">
    15 documented languages across streaming and non-streaming modes, with automatic language detection (`multi-eu` / `multi-indic` / `multi-asian` aggregators) and code-switching within a single session.
  </Card>
...
| Gujarati | `gu` | Yes |
| Marathi | `mr` | Yes |
| Multi-Indic aggregator | `multi-indic` | Yes &mdash; auto-detects across `en`, `hi`, `gu`, `mr` |
