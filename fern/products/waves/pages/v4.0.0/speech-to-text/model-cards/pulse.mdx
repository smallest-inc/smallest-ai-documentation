...
Pulse supports the same individual languages in both streaming and non-streaming modes. Pass the code below as `?language=<code>`.
...
When the spoken language can be one of several, pass an aggregator code instead of an individual one. Pulse routes to the right model internally.
