---
title: Pulse
description: High-accuracy, low-latency speech-to-text model built for real-time transcription across 35 documented languages (21 streaming + 26 pre-recorded), with regional aggregators and both streaming and non-streaming support.
icon: microphone
---
...
| 16 | Oriya | `or` |
| 17 | Bengali | `bn` |
| 18 | Tamil <Badge color="yellow">Beta</Badge> | `ta` &mdash; India region only \[**] |
| 19 | Telugu <Badge color="yellow">Beta</Badge> | `te` &mdash; India region only \[**] |
| 20 | Kannada <Badge color="yellow">Beta</Badge> | `kn` &mdash; India region only \[**] |
| 21 | Malayalam <Badge color="yellow">Beta</Badge> | `ml` &mdash; India region only \[**] |
| 22 | North-Indic aggregator | `north_indic` &mdash; auto-detects across `en`, `hi`, `gu`, `mr`, `bn`, `or` |
| 23 | Multi-Asian aggregator | `multi-asian` &mdash; auto-detects across `zh`, `yue`, `ko`, `ja`, `en`. US region only \[*]. Contact sales for access in the India region. |
| 24 | South-Indic aggregator <Badge color="yellow">Beta</Badge> | `multi-south-indic` &mdash; auto-detects across `ta`, `te`, `kn`, `ml`, and English code-switching. India region only \[**]. Use when the South Indian language is not known in advance. |
...
<Accordion title="Non-Streaming (Pre-Recorded) — 26 languages + 3 regional aggregators">
...
| 29 | Multi-Indic aggregator | `multi-indic` &mdash; auto-detects across `en`, `hi`, `gu`, `mr`, `bn`, `or`. India region only. |
...
Pulse supports both streaming and batch transcription across 35 languages (21 streaming + 26 pre-recorded, 12 codes overlap both modes). Pulse Pro is English-only and batch-only, but achieves higher accuracy — tied for #2 on the Open ASR Leaderboard at 5.42% average WER vs Pulse's 6.03% on English FLEURS. Use Pulse for live streaming, multilingual audio, or latency-sensitive workloads. Use [Pulse Pro](/waves/model-cards/speech-to-text/pulse-pro) for high-volume English batch transcription where accuracy is the top requirement.
</Accordion>
