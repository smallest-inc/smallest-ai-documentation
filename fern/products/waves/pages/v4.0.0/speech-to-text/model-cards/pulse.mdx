...
<Note>
Non-streaming (batch) currently does not officially support `ja`, `yue`, `zh`, or `ko`. Cantonese (`yue`) is not enabled at all for batch; the behaviour of `ja`, `zh`, `ko` on the batch endpoint is separate from streaming and not formally supported — use the streaming endpoint for these four languages.
</Note>
