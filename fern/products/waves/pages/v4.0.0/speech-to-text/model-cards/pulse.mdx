...
| **Pricing (Standard Plan)** | Realtime: $0.008/min, Batch: $0.005/min |
...
<Capabilities bordered>
  ...
  <Capability icon="volume-low" name="Noise Reduction">
...
See the [Pulse quickstart](/waves/documentation/speech-to-text-pulse/quickstart) for a working end-to-end example, including authentication, request shape, and response parsing. Pulse is selected via the `model` query parameter on the unified STT endpoint — `POST /waves/v1/stt/?model=pulse` for pre-recorded transcription, `WSS /waves/v1/stt/live?model=pulse` for streaming.
...
| Keyword boosting | Custom vocabulary enhancement |
| PII redaction | Personal info redaction |
| PCI redaction | Payment card data redaction |
| Word-level timestamps | Per-word timing |
| Sentence-level timestamps | Per-sentence timing |
| Punctuation | Auto punctuation |
| Code-switching | Multi-language in same audio |
...
| PCI redaction | Payment card data redaction |
| Word-level timestamps | Per-word timing |
| Sentence-level timestamps | Requires `word_timestamps=true` to be enabled |
| Punctuation | Auto punctuation |
| Code-switching | Multi-language in same audio |
...
| **Supported sample rates** | 8,000 / 16,000 / 24,000 / 44,100 / 48,000 Hz |
| **Recommended sample rate** | 16,000 Hz (balance of quality and latency) |
| **Audio channels** | Mono (multi-channel audio is mixed down) |
| **Transport** | HTTP (pre-recorded), WebSocket (streaming) |
| **Recommended GPU** (self-host) | NVIDIA L4 (24 GB VRAM); L40S / A100 / H100 supported |
