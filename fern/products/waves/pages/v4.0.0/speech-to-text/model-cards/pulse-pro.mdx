...
| **Hardware** | 1× NVIDIA L40S (48 GB VRAM) |
...
Returns `202` immediately; webhook receives the full transcription when ready.
...
- **RTFx hardware reference:** the public leaderboard measures throughput on A100-80GB. Pulse Pro's published 250–300× is on L40S, which is roughly half the throughput for this workload. On A100 the figure lands materially higher.
...
| `webhook_url` | URL | No | Receive the transcription asynchronously; endpoint returns `202` immediately. |
