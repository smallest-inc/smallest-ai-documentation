...
- **Send pre-recorded audio only.** Pulse Pro has no streaming worker; `wss://.../stt/live?model=pulse-pro` returns `400`. For live audio, use standard [Pulse](/waves/model-cards/speech-to-text/pulse).
- **Use `webhook_url` for long files.** Files longer than a few minutes benefit from async callback delivery — the request returns `200` immediately and the transcription is POSTed to your endpoint when ready.
- **Send 16 kHz mono WAV / FLAC where possible.** Other formats (MP3, Opus, μ-law) are accepted but resampling adds latency.
- **Toggle `word_timestamps` off by default.** Timestamps cost ~33% of throughput; enable only when downstream consumers need word-level alignment.
- **Use English source audio.** Pulse Pro v4.1 is trained only on English. For multilingual audio, swap to standard [Pulse](/waves/model-cards/speech-to-text/pulse).
