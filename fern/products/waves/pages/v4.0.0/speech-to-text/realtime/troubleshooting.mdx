...
These apply to both the WebSocket handshake and any REST call you make to set up a session.

### 401 Unauthorized

Missing or invalid `Authorization` header on the WebSocket handshake or REST call.

**Fix.** Send `Authorization: Bearer <KEY>` with a single space and no quotes. Browsers cannot set custom headers on WebSocket handshakes, so browser clients must instead use the `?token=<KEY>` query parameter where supported. Check the key has not been rotated at [app.smallest.ai/dashboard/api-keys](https://app.smallest.ai/dashboard/api-keys).
...
The request exceeded the current rate limit. Rare on a single WebSocket session but possible when opening many sessions concurrently.
