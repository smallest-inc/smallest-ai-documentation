...
This guide shows you how to transcribe streaming audio using Smallest AI's Pulse STT model via the WebSocket API. Pulse delivers ~150 ms streaming TTFT (Time to First Transcript) at 1 concurrency on production hardware, scaling to ~300 ms at 100 concurrent requests — see [Measuring latency](/waves/documentation/speech-to-text-pulse/measuring-latency) for the methodology and how to attribute the four pipeline components. Suitable for live conversations, voice assistants, and any scenario where you want transcripts to arrive while the speaker is still talking.
...
Head over to the [smallest console](https://app.smallest.ai/dashboard/api-keys?utm_source=documentation&utm_medium=speech-to-text) to generate an API key if not done previously. Also look at [Authentication guide](/waves/api-reference/api-references/authentication) for more information about API keys and their usage.
