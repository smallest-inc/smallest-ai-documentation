...
| `max_tokens` | int | model max | Cap on output tokens. Hard ceiling: 32,768 (combined input + output context). |
...
All errors follow OpenAI's envelope shape:
...
| 429 | Rate limit (RPM) or concurrency cap hit. See [Concurrency and Limits](/waves/api-reference/concurrency-and-limits). |
