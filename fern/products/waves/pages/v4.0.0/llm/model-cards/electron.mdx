...
**Jump to:** [Benchmarks](#performance--benchmarks) · [Supported Languages](#supported-languages) · [API Reference](/waves/api-reference/api-reference/llm-chat-completions/electron-chat-completions) · [Pricing & Throughput](#throughput-latency--pricing) · [Quickstart](/waves/documentation/llm-electron/quickstart)
...
See the [Electron quickstart](/waves/documentation/llm-electron/quickstart) for a working end-to-end example, including authentication, request shape, and streaming response handling. Electron is OpenAI-wire-compatible — swap `base_url` to `https://api.smallest.ai/waves/v1` on the official OpenAI SDK, pass your `SMALLEST_API_KEY` as `api_key`, and set `"model": "electron"` on the request body.
...
See [Electron — Chat Completions](/waves/api-reference/api-reference/llm-chat-completions/electron-chat-completions) for the full request/response schema, supported parameters, and error codes. The [Chat Completions guide](/waves/documentation/llm-electron/chat-completions) covers the OpenAI-compatible wire format end-to-end, and [Supported Parameters](/waves/documentation/llm-electron/supported-parameters) lists the passthrough table.
...
Both limits enforce strictly — over either cap returns `HTTP 429`. See [Concurrency & Limits](/waves/api-reference/api-references/concurrency-and-limits) for full rate-limit semantics.

Pricing: Contact your Smallest AI account manager. Prefix-cache discounts apply automatically — see [Prefix Caching](/waves/documentation/llm-electron/prefix-caching). Every response reports `usage.prompt_tokens_details.cached_tokens` so you can audit cache hit rates.
...
- **Stream when you can.** Set `"stream": true` and start your TTS engine on the first `delta.content` chunk to mask end-to-end latency. See [Streaming](/waves/documentation/llm-electron/streaming).
- **Put repeated context at the prompt prefix.** System prompts, RAG context, and conversation history live in the cached prefix automatically. See [Prefix Caching](/waves/documentation/llm-electron/prefix-caching).
- **For voice agents, prompt for a filler phrase before tool calls.** Electron emits the filler in `content` alongside `tool_calls`, so your TTS can speak it while the tool runs. See [Tool Calling](/waves/documentation/llm-electron/tool-function-calling).
