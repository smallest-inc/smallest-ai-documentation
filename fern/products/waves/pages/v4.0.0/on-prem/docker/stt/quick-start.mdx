...
<Note>
  Ensure you've completed all [prerequisites](/models/self-host/docker-setup/stt-deployment/prerequisites/hardware-requirements) before
  starting this guide.
</Note>
    ...
    services:
      ...
      api-server:
        ...
        environment:
          ...
          - LIGHTNING_ASR_STREAMING_BASE_URL=http://lightning-asr:2233
...
<Steps>
  ...
  <Step title="License Proxy Ready">
    ...
    license-proxy  | Server listening on port 6699
    ```
  </Step>
...
<CardGroup cols={2}>
  <Card title="STT Configuration" href="/models/self-host/docker-setup/stt-deployment/configuration">
    Customize your deployment with advanced configuration options
  </Card>

  <Card title="STT Services Overview" href="/models/self-host/docker-setup/stt-deployment/services-overview">
    Learn about each service component in detail
  </Card>

  <Card title="STT Troubleshooting" href="/models/self-host/docker-setup/stt-deployment/troubleshooting">
    Debug common issues and optimize performance
  </Card>

  <Card title="API Reference" href="/models/self-host/api-reference/authentication">
