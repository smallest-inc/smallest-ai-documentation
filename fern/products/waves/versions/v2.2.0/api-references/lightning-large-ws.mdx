...
const socket = new WebSocket(
  'wss://api.smallest.ai/waves/v1/lightning-large/stream'
);
```
...
const ws = new WebSocket(
  'wss://api.smallest.ai/waves/v1/lightning-large/get_speech/stream',
  {
    headers: {
      Authorization: 'Bearer YOUR_API_KEY'
    }
...
WS_URL = "wss://api.smallest.ai/waves/v1/lightning-large/get_speech/stream"
...
async def text_to_speech():
    async with websockets.connect(WS_URL, extra_headers={"Authorization": f"Bearer {API_KEY}"}) as ws:
        ...
        while True:
            ...
            elif response_data["status"] == "sucompleteccess" and response_data.get("done", False):
