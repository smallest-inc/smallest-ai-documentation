...
| --- | --- | --- | --- |
| `api_key` | `str` | `$SMALLEST_API_KEY` | Your Smallest AI API key |
| `model` | `str` | `"lightning-v3.1"` | TTS model. Use `"lightning-v3.1"` — the current model with 80+ voices and ~100ms latency |
| `voice_id` | `str` | `"sophia"` | Voice ID for synthesis |
| `language` | `str` | `"en"` | Language code. 22 supported codes plus `"auto"` — see the [Lightning v3.1 model card](/waves/documentation/text-to-speech-lightning/lightning-v-3-1#supported-languages) for the full list. |
...
- `"lightning-v3.1"` is the current TTS model — it delivers ~100ms latency with 80+ voices.
