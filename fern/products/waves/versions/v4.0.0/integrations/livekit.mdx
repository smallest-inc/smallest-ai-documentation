...
<Note>
The plugin example below targets the standard Lightning v3.1 pool. To use the [Lightning v3.1 Pro](/waves/model-cards/text-to-speech/lightning-v-3-1-pro) pool today, connect to `wss://api.smallest.ai/waves/v1/tts/live` directly with the WebSocket example on the Pro model card and pass `"model": "lightning_v3.1_pro"` in the payload — `livekit-plugins-smallestai` will add a Pro option in a future release.
</Note>
