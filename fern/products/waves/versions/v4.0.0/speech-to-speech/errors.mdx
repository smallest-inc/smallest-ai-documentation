...
An error frame has both a `type` (broad category) and a `code` (specific reason). Match on **`error.code`** for actionable handling — `error.type` is `invalid_request_error` for almost every client-induced failure.
...
|---|---|---|
| `invalid_request_error` | Schema validation failure — unknown field, missing `type`, wrong type, malformed JSON, unsupported audio format. | Fix the frame and resend. Session continues. |
...
Note that `invalid_request_error` shows up in *both* fields: it's always the `error.type`, and it's also the `error.code` value when the failure is generic schema validation rather than a more specific bucket like `invalid_audio`.
...
See [WebSocket connection](/waves/documentation/speech-to-speech-hydra/websocket-connection#close-codes) for the full close-code reference.
...
- [WebSocket connection](/waves/documentation/speech-to-speech-hydra/websocket-connection) — close codes and idle timeout
