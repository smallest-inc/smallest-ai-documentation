...
Flip one query parameter and the same Pulse STT WebSocket also emits acoustic voice-activity events — `speech_started` and `speech_ended` — interleaved with your normal `transcription` messages. Useful for "is the user talking?" UI state, barge-in detection, mic visualization, and silence-driven turn boundaries that don't depend on transcript finalization.
...
- **Acoustic, not lexical.** These are real voice-activity boundaries from the audio. They're independent of transcript finalization (`is_final`), so they won't line up exactly with words — that's expected.
