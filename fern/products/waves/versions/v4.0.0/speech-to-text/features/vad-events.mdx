...
- **`speech_ended` requires trailing silence.** The event is emitted when silence is detected after a voiced region. If the connection closes while the audio still contains voiced energy — for example, sending `close_stream` immediately after the last word of an audio file — no `speech_ended` is emitted for the final voiced region. To force emission at the end of a finite audio file, append a short pad (≈1 s) of zero-valued PCM before sending `close_stream`.
