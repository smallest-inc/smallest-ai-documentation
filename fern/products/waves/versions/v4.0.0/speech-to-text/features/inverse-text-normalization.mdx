...
3. When your VAD detects the user has stopped speaking, choose the control message that matches your session shape:
   - **`{"type": "finalize"}`** — for **multi-turn voice agents**. Flushes the current buffer, runs ITN over the full utterance, and emits an `is_final: true` transcript while **keeping the WebSocket open** for the next user turn. Use this between every turn of a conversation.
   - **`{"type": "close_stream"}`** — for **single-shot transcription** (or at the end of a multi-turn session). Flushes any remaining audio, emits the terminal `is_final: true` + `is_last: true` transcript, then closes the session.

**Rule of thumb:** if the same user will speak again on the same WebSocket, use `finalize`. If the session is over, use `close_stream`. A typical voice agent sends many `finalize` messages and a single `close_stream` at the end.
...
async def transcribe(audio_file: str):
    ...
    async with websockets.connect(WS_URL, additional_headers=headers) as ws:
        ...
        # Signal end of audio (close_stream is the only message that emits is_last=true)
