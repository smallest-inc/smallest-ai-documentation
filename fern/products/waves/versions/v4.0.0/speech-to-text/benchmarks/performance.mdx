...
These four languages stream from `wss://api.us.smallest.ai/waves/v1/stt/live?model=pulse` only (US region). See the [Pulse model card](/waves/model-cards/speech-to-text/pulse#supported-languages--streaming) for the region-routing details.
