...
Our evaluation guide outlines a repeatable process: choose representative audio, generate transcripts, compute WER/CER/latency, and document findings. The ready-to-run snippets below call the [Pulse pre-recorded REST endpoint](/waves/api-reference/api-reference/speech-to-text/pulse-speech-to-text) directly so you can audit the methodology and adapt it to any HTTP client.
...
The Pulse pre-recorded endpoint accepts the raw audio bytes as `application/octet-stream` and returns JSON with `transcription`, `words[]`, `utterances[]`, and `metadata`. Latency is measured client-side; Real-Time Factor (RTF) is `audio_duration ÷ wall_time`.
...
def run_sample(sample: dict) -> dict:
    ...
    rtf = duration_s / (latency_ms / 1000)
...
- **RTF** from `rtf` — values above 1.0 mean Pulse processed audio faster than real-time.
...
def evaluate_config(config: dict, dataset: list) -> pd.DataFrame:
    overridden = [{**s, **{k: v for k, v in config.items() if k in {"language"}}} for s in dataset]
