...
Our evaluation guide outlines a repeatable process: choose representative audio, generate transcripts, compute WER/CER/latency, and document findings. The ready-to-run snippets below call the [Pulse pre-recorded REST endpoint](/models/api-reference/api-reference/speech-to-text/transcribe) directly so you can audit the methodology and adapt it to any HTTP client.
...
- **RTF** from `rtf` — values **below 1.0** mean Pulse processed audio faster than real-time. Pulse STT typically runs near 0.4 on clean inputs (see [Metrics Overview](/models/documentation/speech-to-text-pulse/benchmarks/metrics-overview#real-time-factor-rtf)).
...
- For proper-noun mistakes, queue them for a [pronunciation dictionary](/models/documentation/text-to-speech-lightning/pronunciation-dictionaries) if you also use Lightning TTS, or for the Pulse [keyword boosting](/models/documentation/speech-to-text-pulse/features/keyword-boosting) feature on the streaming endpoint.
