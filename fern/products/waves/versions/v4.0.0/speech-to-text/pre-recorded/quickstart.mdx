...
This guide shows you how to convert an audio file into text using the unified Speech-to-Text endpoint. Both Pulse (multilingual, 17 streaming + 26 pre-recorded languages) and Pulse Pro (leaderboard-ranked English) live behind the same path; you pick the model with `?model=`.
...
| Multilingual, or you need streaming | **`pulse`** | 17 streaming + 26 pre-recorded languages, runs on both HTTP and the live WebSocket endpoint. |
...
For non-English audio, code-switching, or when you need streaming, use `?model=pulse`. Set `language` explicitly to the known code (`en`, `hi`, `es`, etc.) for best accuracy, or use a regional aggregator (`multi-eu`, `multi-indic`) for unknown audio. See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for the full per-mode language matrix.
...
<Note>
For Pulse, set `language` explicitly to match the audio (`en`, `hi`, `es`, etc.) for the best accuracy. For unknown audio, pick the regional auto-detect scope (`multi-eu`, `multi-indic`). See the [Pulse model card](/waves/model-cards/speech-to-text/pulse) for which codes each aggregator covers.
</Note>
