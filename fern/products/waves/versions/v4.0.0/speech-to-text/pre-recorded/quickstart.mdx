...
This guide shows you how to convert an audio file into text using the unified Speech-to-Text endpoint. Both Pulse (multilingual, 15 languages) and Pulse Pro (leaderboard-ranked English) live behind the same path; you pick the model with `?model=`.
...
| Multilingual, or you need streaming | **`pulse`** | 15 languages plus `multi-eu` / `multi-indic` aggregators. Runs on both HTTP and the live WebSocket endpoint. |
...
Head over to the [smallest console](https://app.smallest.ai/dashboard/api-keys) to generate an API key, if not done previously. Also look at the [Authentication guide](/waves/documentation/getting-started/authentication) for more information about API keys.
...
<Note>
For Pulse, set `language` explicitly to match the audio (`en`, `hi`, `de`, `es`, `ru`, `it`, `fr`, `nl`, `pt`, `zh`, `yue`, `ja`, `ko`, `gu`, `mr`) for best accuracy. For mixed audio, pick an aggregator: `multi-eu` (en + de + es + it + pt + nl + ru + fr) or `multi-indic` (en + hi + gu + mr).
</Note>
