...
- **Trying to derive a per-chunk "live lag" number on the client.** For live-captioning workloads, use the cookbook scripts (per-word emission timing) or live side-by-side testing against real users — that's what AssemblyAI, Pipecat, and Daily.co recommend.
