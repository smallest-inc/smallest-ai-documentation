...
1. **End-of-turn (EOT) latency** — time between a speaker stopping and the final transcript arriving. The metric voice agents are judged on. Also called Time-to-Final-Segment (TTFS) by the [Pipecat STT benchmark](https://www.daily.co/blog/benchmarking-stt-for-voice-agents/) or Time-to-Complete-Transcript (TTCT) by [AssemblyAI](https://www.assemblyai.com/docs/streaming/evaluations/voice-agents).
...
<Note>
**No "transcript latency" cursor-difference formula here.** Some providers document a `audio_cursor − transcript_cursor` formula as a measure of "live transcript lag" — but it inflates artificially during silence in the audio (the audio cursor advances while the transcript cursor correctly sits still on the last word) and isn't part of any industry-standard benchmark. The [Pipecat STT benchmark](https://www.daily.co/blog/benchmarking-stt-for-voice-agents/) used across the voice-agent industry measures TTFS / EOT only. AssemblyAI documents TTFT and TTCT and recommends [live side-by-side testing](https://www.assemblyai.com/docs/streaming/evaluations/voice-agents) rather than per-chunk formulas. We follow the same approach.
</Note>
...
- **Trying to invent a "live transcript lag" formula from cursors.** Don't — see the note at the top of the page. Use the cookbook scripts or live side-by-side testing for live-captioning workloads.
