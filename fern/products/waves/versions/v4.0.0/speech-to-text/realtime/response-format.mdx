...
- The `language` field is set to the specified language, or the detected language if `language` is set to a regional aggregator (`north_indic` or `multi-asian` on streaming). Other responses will not include the `language` field.
