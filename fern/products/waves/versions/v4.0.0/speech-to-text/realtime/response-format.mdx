...
- The `language` field is set to the specified language, or the detected language if `language` is set to a regional aggregator (`north_indic`, `multi-asian`, or `multi-south-indic` on streaming). Other responses will not include the `language` field.
